...
```bash
# 1. Set up test directories (one-time setup)
./tools/setup_test_dirs.sh

# 2. Build all binaries
cd build_test && make circle -j$(nproc) && cd ..
cd build_sqlite && cmake -DHAVE_SQLITE=ON .. && make circle -j$(nproc) && cd ..
cd build_yaml && cmake -DHAVE_YAML=ON .. && make circle -j$(nproc) && cd ..

# 3. Run quick comparison test (recommended for development)
