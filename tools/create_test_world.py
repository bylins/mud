...
Ключи:
  -w, --world-dir   каталог исходного мира (по умолчанию lib/world)
  -o, --output      каталог для тестового мира (по умолчанию ./world.test)
  -f, --format      auto|legacy|yaml (по умолчанию auto)
  --min / --max     границы диапазона зон (по умолчанию 1 / 0)
  --include         зоны, которые копируются всегда; список с диапазонами,
                    напр. «1,40,600-699»
  --exclude         зоны, которые не копировать; список с диапазонами
                    (по умолчанию «0»)
  --clean           очистить каталог вывода перед копированием
...
def main(argv=None):
    ...
    parser.add_argument("--include", type=parse_zone_list, default=set(),
                        help="зоны, которые копируются всегда; список с диапазонами, "
                             "напр. «1,40,600-699»")
    parser.add_argument("--exclude", type=parse_zone_list, default={0},
                        help="зоны, которые не копировать; список с диапазонами "
                             "(по умолчанию «0»)")
