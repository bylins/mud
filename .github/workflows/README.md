...
```
meson setup build -Dbuild_profile=dev -Dbuild_tests=false -Dunity_size=45
...
```bash
meson setup build -Dbuild_profile=release -Dunity_size=45
meson compile -C build
meson test -C build
```
...
```bash
meson setup build -Dbuild_profile=release -Dyaml=system -Dunity_size=45
meson compile -C build
meson test -C build
```
...
```bash
meson setup build -Dbuild_profile=release -Dsqlite=system -Dunity_size=45
meson compile -C build
meson test -C build
```
...
```bash
meson setup build -Dbuild_profile=release -Dadmin_api=true -Dotel=system -Dunity_size=45
meson compile -C build
meson test -C build
```
