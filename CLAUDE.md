...
- **major.minor.patch** are declared explicitly in the repo-root **`VERSION`** file — the single source of truth.
...
- **patch** — bump **manually and without prompting** on **every bug fix and every feature-branch merge**. Count the merges: if two branches are merged (e.g. one into `master` and one into `unstable`), increase patch by **2**. Treat editing `VERSION` as a routine step of finishing such work, included in the same commit/merge.
...
Only the `VERSION` file changes for a version bump; `release` and the build-date stamp update themselves on the next build.
