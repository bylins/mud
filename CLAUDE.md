...
## Working with KOI8-R Encoded Files

**CRITICAL**: Most source files use **KOI8-R** encoding (old Russian encoding), NOT UTF-8. Git stores them as KOI8-R, but may display them incorrectly.

### Files in KOI8-R encoding:
- `src/**/*.cpp` and `*.h` - All C++ source files with Russian comments
- `tests/CMakeLists.txt` - Test configuration with Russian comments
- Any file with Russian text in comments

### How to identify KOI8-R vs UTF-8:
```bash
# Check hex bytes of Russian text:
# KOI8-R: f0 cf c9 d3 cb (for "Поиск")
# UTF-8:  d0 9f d0 be d0 b8 d1 81 d0 ba (for "Поиск")

sed -n '5p' file.cpp | hexdump -C
```

### Rules for editing KOI8-R files:

**ALWAYS use `sed` for text modifications** - it preserves encoding:
```bash
# Good - preserves KOI8-R encoding
sed -i 's/old/new/' file.cpp
sed -i '10,20d' file.cpp  # Delete lines

# BAD - corrupts encoding
# Edit tool / Write tool - will convert to UTF-8
```

### Safe workflow:

1. **For small changes**: Use `sed` with line numbers
2. **For new English comments/code**: Safe to use sed or manual tools
3. **For changes near Russian text**: Use sed only
4. **Always verify** after changes:
```bash
# Check encoding not corrupted:
file file.cpp  # Should show "ISO-8859 text" for KOI8-R
head file.cpp | iconv -f koi8-r -t utf-8  # Should display correctly
```

### Converting between encodings:

```bash
# UTF-8 → KOI8-R (when you accidentally created UTF-8 file):
iconv -f utf-8 -t koi8-r input.cpp > output.cpp

# KOI8-R → UTF-8 (for display/reading only):
iconv -f koi8-r -t utf-8 file.cpp | less

# Verify hex bytes:
sed -n '5p' file.cpp | hexdump -C | head -2
```

### If encoding gets corrupted:

1. **Check git for clean version**:
   ```bash
   git show HEAD~1:path/to/file.cpp > /tmp/clean.cpp
   ```

2. **If git version is UTF-8, convert to KOI8-R**:
   ```bash
   git show origin/master:file.cpp | iconv -f utf-8 -t koi8-r > file.cpp
   ```

3. **Reapply only functional changes** using `sed`

### Example - safe replacement:
```bash
# Change add_test line without corrupting Russian comments:
sed -i '100s|add_test(all tests)|add_test(NAME all COMMAND tests)|' CMakeLists.txt

# Verify encoding preserved:
file CMakeLists.txt  # Should be ISO-8859
sed -n '5p' CMakeLists.txt | hexdump -C  # Check Russian text bytes
```

**Remember**: `file` command reports KOI8-R as "ISO-8859 text" - this is normal and correct!
