...
int check_death_ice(int room, CHAR_DATA * ch)
{
	...
	for (vict = world[room]->people; vict; vict = vict->next_in_room)
		if (!IS_NPC(vict) && !AFF_FLAGGED(vict, EAffectFlags::AFF_FLY))
...
int has_boat(CHAR_DATA * ch)
{
	...
	if (AFF_FLAGGED(ch, EAffectFlags::AFF_WATERWALK))
		return (TRUE);
...
void make_visible(CHAR_DATA * ch, const EAffectFlags affect)
{
...
int skip_hiding(CHAR_DATA * ch, CHAR_DATA * vict)
{
	...
	if (MAY_SEE(ch, vict, ch) && (AFF_FLAGGED(ch, EAffectFlags::AFF_HIDE) || affected_by_spell(ch, SPELL_HIDE)))
	{
		if (awake_hide(ch))  	//if (affected_by_spell(ch, SPELL_HIDE))
		{
			...
			make_visible(ch, EAffectFlags::AFF_HIDE);
...
int skip_camouflage(CHAR_DATA * ch, CHAR_DATA * vict)
{
			...
			|| affected_by_spell(ch, SPELL_CAMOUFLAGE)))
	{
		if (awake_camouflage(ch))  	//if (affected_by_spell(ch,SPELL_CAMOUFLAGE))
		{
			...
			make_visible(ch, EAffectFlags::AFF_CAMOUFLAGE);
		...
		else if (affected_by_spell(ch, SPELL_CAMOUFLAGE))
		{
			...
			{
				affect_from_char(ch, SPELL_CAMOUFLAGE);
				if (!AFF_FLAGGED(ch, EAffectFlags::AFF_CAMOUFLAGE))
				{
...
int skip_sneaking(CHAR_DATA * ch, CHAR_DATA * vict)
{
	...
	if (MAY_SEE(ch, vict, ch) && (AFF_FLAGGED(ch, EAffectFlags::AFF_SNEAK) || affected_by_spell(ch, SPELL_SNEAK)))
	{
		if (awake_sneak(ch))  	//if (affected_by_spell(ch,SPELL_SNEAK))
		{
			...
			if (affected_by_spell(ch, SPELL_HIDE))
				affect_from_char(ch, SPELL_HIDE);
			make_visible(ch, EAffectFlags::AFF_SNEAK);
...
int legal_dir(CHAR_DATA * ch, int dir, int need_specials_check, int show_msg)
{
	...
	// charmed
	if (AFF_FLAGGED(ch, EAffectFlags::AFF_CHARM) && ch->master && ch->in_room == ch->master->in_room)
	{
		if (show_msg)
		{
	...
	if (IS_NPC(ch))
	{
		...
		if (!MOB_FLAGGED(ch, MOB_SWIMMING) &&
				...
				!AFF_FLAGGED(ch, EAffectFlags::AFF_FLY) &&
		...
		if (!MOB_FLAGGED(ch, MOB_FLYING) &&
				!AFF_FLAGGED(ch, EAffectFlags::AFF_FLY) && SECT(EXIT(ch, dir)->to_room) == SECT_FLYING)
			return (FALSE);
		...
		if (ROOM_FLAGGED(EXIT(ch, dir)->to_room, ROOM_NOMOB) &&
				!IS_HORSE(ch) &&
				!AFF_FLAGGED(ch, EAffectFlags::AFF_CHARM) && !MOB_FLAGGED(ch, MOB_ANGEL) && !MOB_FLAGGED(ch, MOB_IGNORNOMOB))
			return (FALSE);
	...
	else
	{
		...
		if (on_horse(ch)
			&& (AFF_FLAGGED(get_horse(ch), EAffectFlags::AFF_HOLD)
				|| AFF_FLAGGED(get_horse(ch), EAffectFlags::AFF_SLEEP)))
		{
			if (show_msg)
		...
		for (tch = world[IN_ROOM(ch)]->people; tch; tch = tch->next_in_room)
		{
			...
			if (NPC_FLAGGED(tch, 1 << dir)
				...
				&& CAN_SEE(tch, ch)
				&& !AFF_FLAGGED(tch, EAffectFlags::AFF_CHARM)
				&& !AFF_FLAGGED(tch, EAffectFlags::AFF_HOLD))
			{
				if (show_msg)
...
int do_simple_move(CHAR_DATA * ch, int dir, int need_specials_check, CHAR_DATA * leader)
{
	...
	if (AFF_FLAGGED(ch, EAffectFlags::AFF_SNEAK) && !IsFlee)
	{
		if (IS_NPC(ch))
			invis = 1;
	...
	if (AFF_FLAGGED(ch, EAffectFlags::AFF_CAMOUFLAGE) && !IsFlee)
	{
		if (IS_NPC(ch))
			invis = 1;
	...
	direction = dir + 1;
	use_horse = AFF_FLAGGED(ch, EAffectFlags::AFF_HORSE)
		&& has_horse(ch, FALSE)
		&& (IN_ROOM(get_horse(ch)) == was_in
			|| IN_ROOM(get_horse(ch)) == go_to);
	is_horse = IS_HORSE(ch)
		&& (ch->master)
		&& !AFF_FLAGGED(ch->master, EAffectFlags::AFF_INVISIBLE)
	...
	if (!invis && !is_horse)
	{
		...
		else if ((!use_horse && AFF_FLAGGED(ch, EAffectFlags::AFF_FLY))
		...
		else if (use_horse)
		{
			...
			if (horse && AFF_FLAGGED(horse, EAffectFlags::AFF_FLY))
			{
				strcpy(buf1, "улетел$g");
			}
			else
...
*/
	...
	else
	{
		...
		// add track info
		if (!AFF_FLAGGED(ch, EAffectFlags::AFF_NOTRACK) && (!IS_NPC(ch) || (mob_rnum = GET_MOB_RNUM(ch)) >= 0))
		{
			...
			if (track)
			{
				...
				if (affected_by_spell(ch, SPELL_LIGHT_WALK) && !on_horse(ch))
					if (AFF_FLAGGED(ch, EAffectFlags::AFF_LIGHT_WALK))
	...
	for (vict = world[IN_ROOM(ch)]->people; !IS_NPC(ch) && vict; vict = vict->next_in_room)
	{
		...
		if (!CAN_SEE(vict, ch) ||
				...
				AFF_FLAGGED(ch, EAffectFlags::AFF_CAMOUFLAGE) || vict->get_fighting() || GET_POS(vict) < POS_RESTING)
			continue;

		// AWARE mobs
		if (MOB_FLAGGED(vict, MOB_AWARE) &&
				GET_POS(vict) < POS_FIGHTING && !AFF_FLAGGED(vict, EAffectFlags::AFF_HOLD) && GET_POS(vict) > POS_SLEEPING)
		{
		...
		else
		{
			...
			for (k = ch->followers; k && k->follower->master; k = next)
			{
				...
				if (k->follower->in_room == was_in
					...
					&& (!IS_HORSE(k->follower)
						|| !AFF_FLAGGED(k->follower, EAffectFlags::AFF_TETHERED)))
				{
		...
		if (isname(buf, p_str))
		{
			...
			else
			{
				...
				if (!IS_IMMORTAL(ch) && ((!IS_NPC(ch) && (!Clan::MayEnter(ch, door, HCE_PORTAL)
											...
											|| AFF_FLAGGED(ch, EAffectFlags::AFF_NOTELEPORT)
...
ACMD(do_stand)
{
	if (GET_POS(ch) > POS_SLEEPING && AFF_FLAGGED(ch, EAffectFlags::AFF_SLEEP))
	{
...
ACMD(do_horseon)
{
	...
	else if (AFF_FLAGGED(horse, EAffectFlags::AFF_TETHERED))
...
ACMD(do_horsetake)
{
	...
	else if (!IS_GOD(ch) && !MOB_FLAGGED(horse, MOB_MOUNTING) && !((horse->master) && AFF_FLAGGED(horse, EAffectFlags::AFF_HORSE)))
	{
	...
	else if (IS_HORSE(horse))
	{
		...
		if (!IS_IMMORTAL(ch))
		{
			...
			if (AWAKE(horse->master) && (IN_ROOM(ch) == IN_ROOM(horse->master)))
				percent += 50;
			if (AFF_FLAGGED(horse, EAffectFlags::AFF_TETHERED))
				percent += 10;
...
ACMD(do_follow)
{
	...
	if (IS_NPC(ch) && AFF_FLAGGED(ch, EAffectFlags::AFF_CHARM) && ch->get_fighting())
		return;
	...
	if (AFF_FLAGGED(ch, EAffectFlags::AFF_CHARM) && (ch->master))
	{
