 ...
 */
const char *kValueAttribute = "val";
...
const char *kBaseCharacteristicEntity = "base_characteristic";
const char *kSavingEntity = "saving";
const char *kAbbreviationEntity = "abbreviation";
const char *kDifficultyEntity = "difficulty";
const char *kCritFailEntity = "critical_fail_threshold";
const char *kCritSuccessEntity = "critical_success_threshold";
const char *kMobVsPcPenaltyEntity = "mob_vs_pc_penalty";
const char *kPcVsPcPenaltyEntity = "pc_vs_pc_penalty";
const char *kMessagesEntity = "messages";
const char *kMsgEntity = "msg";
const char *kCircimstancesEntity = "circumstances";
const char *kCircimstanceEntity = "circumstance";
const char *kInvertedAttribute = "inverted";
...
AbilitiesInfo::AbilitiesInfoBuilder::CharacteristicGettersRegister AbilitiesInfo::AbilitiesInfoBuilder::characteristic_getters_register_;
...
void AbilitiesInfo::Reload() {
	...
	if (new_abilities) {
		...
		err_log("Abilities reloading canceled: file damaged.");
	}
}
...
void AbilitiesInfo::AbilitiesInfoBuilder::ParseAbilityData(AbilityPtr &ability, const pugi::xml_node &node) {
	ParseBase(ability, node);
	ParseCircumstances(ability, node);
	ParseMessages(ability, node);
}
void AbilitiesInfo::AbilitiesInfoBuilder::ParseBase(AbilityPtr &ability, const xml_node &node) {
	ability->id_ = FindConstantByName<EAbility>(kValueAttribute, node.child(kIdEntity));
...
//	ability->saving_id_ = FindConstantByName<ESaving>(kValueAttribute, node.child(kSavingEntity));
	...
	ability->critical_fail_threshold_ = node.child(kCritFailEntity).attribute(kValueAttribute).as_int();
	ability->critical_success_threshold_ = node.child(kCritSuccessEntity).attribute(kValueAttribute).as_int();
	ability->mob_vs_pc_penalty_ = node.child(kMobVsPcPenaltyEntity).attribute(kValueAttribute).as_int();
	ability->pc_vs_pc_penalty_ = node.child(kPcVsPcPenaltyEntity).attribute(kValueAttribute).as_int();
}
...
/*	auto mod = node.attribute(kValueAttribute).as_int();
	auto inverted = node.attribute(kInvertedAttribute).as_bool();
	auto handler_id = FindConstantByName<ECirumstance>(kIdAttribute, node);
	...
	ability->circumstance_handlers_.front().mod_ = node.attribute(kValueAttribute).as_int();
	ability->circumstance_handlers_.front().inverted_ = node.attribute(kInvertedAttribute).as_bool();
}
...
void AbilitiesInfo::AbilitiesInfoBuilder::ParseMessages(AbilityPtr &ability, const xml_node &node) {
	...
	for (; msg_node; msg_node = msg_node.next_sibling(kMsgEntity)) {
		auto msg_id = FindConstantByName<EAbilityMsg>(kIdAttribute, msg_node);
		auto it = ability->messages_.try_emplace(msg_id, msg_node.attribute(kValueAttribute).value());
		if (!it.second) {
...
void AbilitiesInfo::AbilitiesInfoBuilder::ProcessLoadErrors(const AbilityPtr &ability, std::exception const &e) {
	err_log("Incorrect value or message type '%s' detected in ability '%s'. Item has been skipped.",
...
void AbilitiesInfo::AbilitiesInfoBuilder::EmplaceAbility(AbilitiesRegisterPtr &abilities, AbilityPtr &ability) {
	...
	if (!it.second) {
		err_log("Ability '%s' has already exist. Redundant definition has been ignored.\n",
...
/// \todo Если убирать "принт", то есть смысл убрать и сохранение идов, добавлять сразу геттер
...
void AbilitiesInfo::AbilitiesInfoBuilder::AppointHandlers(AbilitiesRegisterPtr &abilities) {
	for (auto &item : *abilities) {
		item.second->base_characteristic_getter_ =
			characteristic_getters_register_[item.second->GetBaseCharacteristicId()];
...
/*	if (saving_getters_register_.empty()) {
		saving_getters_register_[kSavingWill] = GET_TOTAL_SAVING_WILL;
		saving_getters_register_[kSavingReflex] = GET_TOTAL_SAVING_REFLEX;
		saving_getters_register_[kSavingCritical] = GET_TOTAL_SAVING_CRITICAL;
		saving_getters_register_[kSavingStability] = GET_TOTAL_SAVING_STABILITY;
	}*/
...
/*		   << " Id: " << KGRN << NAME_BY_ITEM<EAbility>(GetId()) << KNRM << "\n"
		   ...
		   << " Base characteristic: " << KGRN << NAME_BY_ITEM<EStat>(GetBaseCharacteristicId()) << KNRM
		   << "\n"
...
const std::string &AbilityInfo::GetMessage(const EAbilityMsg msg_id) const {
	try {
		return messages_.at(msg_id);
	} catch (const std::out_of_range &) {
		return GetDefaultMessage(msg_id);
	}
}

const std::string &AbilityInfo::GetDefaultMessage(const EAbilityMsg msg_id) const {
	try {
		return default_messages_.at(msg_id);
	} catch (const std::out_of_range &) {
		err_log("MsgId type '%s' not found in ability '%s' or in default messages!",
