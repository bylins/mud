...
void go_slay(CharData *ch, CharData *vict) {
	...
	if (!success) {
		...
		if (GetRealStr(ch) < 50) {
		dam = (number(ceil(((GetRealStr(ch) + weapon_dmg) / 1.2) * (ceil GET_SKILL(ch,ESkill::kSlay) / 10) / 1.25),
					 ceil(((GetRealStr(ch) + weapon_dmg) / 1.2) * (ceil GET_SKILL(ch,ESkill::kSlay) / 10) * 1.25)) *
						GetRealLevel(ch)) / 30;
		} else {
