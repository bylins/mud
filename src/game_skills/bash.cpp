	...
	if (!success) {
		if (ch->GetSkill(ESkill::kShieldBash)) {
			...
			if (successShB && GET_EQ(ch, kShield)) {
				...
				dam = number(ceil((((GET_REAL_SIZE(ch) * ((GET_OBJ_WEIGHT(GET_EQ(ch, EEquipPos::kShield))) * 1.5)) / 5) + (GET_SKILL(ch,ESkill::kShieldBash) * 5)) / 1.25),
							 ceil((((GET_REAL_SIZE(ch) * ((GET_OBJ_WEIGHT(GET_EQ(ch, EEquipPos::kShield))) * 1.5)) / 5) + (GET_SKILL(ch,ESkill::kShieldBash) * 5)) * 1.25)) *
					  GetRealLevel(ch) / 30;
