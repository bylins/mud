...
void do_manadrain(CharacterData *ch, char *argument, int/* cmd*/, int/* subcmd*/) {
	...
	if (IS_NPC(ch) || !ch->get_skill(ESkill::SKILL_MANADRAIN)) {
		send_to_char("Вы не знаете как.\r\n", ch);
		return;
	}

	if (IsTimedBySkill(ch, ESkill::SKILL_MANADRAIN) || ch->haveCooldown(ESkill::SKILL_MANADRAIN)) {
	...
	skill = ch->get_skill(ESkill::SKILL_MANADRAIN);

	percent = number(1, MUD::Skills()[ESkill::SKILL_MANADRAIN].difficulty);
	...
	if (!IS_IMMORTAL(ch)) {
		timed.skill = ESkill::SKILL_MANADRAIN;
		timed.time = 6 - MIN(4, (ch->get_skill(ESkill::SKILL_MANADRAIN) + 30) / 50);
