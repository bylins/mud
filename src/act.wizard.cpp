...
room_rnum find_target_room(CHAR_DATA * ch, char *rawroomstr, int trig)
{
	...
	if (isdigit_wrapped(*roomstr) && !strchr(roomstr, '.'))
	{
		tmp = atoi(roomstr);
...
ACMD(do_load)
{
	...
	if (!*buf || !*buf2 || !isdigit_wrapped(*buf2))
	{
...
ACMD(do_liblist)
{
	...
	first = atoi(buf);
	if (*buf2 && isdigit_wrapped(buf2[0]))
	{
		last = atoi(buf2);
	}
	else
...
ACMD(do_forcetime)
{
	...
	for (ca = strtok(argument, " "); ca; ca = strtok(NULL, " "))
	{
		...
		if (m == 'h')	// hours
			...
			m = 60;
		else if (m == 's' || isdigit_wrapped(m))	// seconds
			m = 1;
		else
			m = 0;
		if ((m *= atoi(ca)) > 0)
