...
enum class EExtraFlag : uint32_t {
	...
	ITEM_FREE_FOR_USE = INT_ONE | (1 << 16),    // пока свободно, можно использовать
...
struct index_data {
	...
	index_data(int _vnum)
		: vnum(_vnum), number(0), stored(0), func(NULL), farg(NULL), proto(NULL), zone(0), set_idx(-1) {}
