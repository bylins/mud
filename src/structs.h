...
struct time_data {
	...
	int
	 played;		/* This is the total accumulated time played in secs */
};
...
struct char_player_data {
	...
	int
	 hometown;		/* PC s Hometown (zone)                 */
	struct time_data
	 time;			/* PC's AGE in days                 */
...
struct char_point_data {
	...
	int
	 gold;			/* Money carried                           */
	long
	 bank_gold;		/* Gold the char has in a bank account    */
	long
	 exp;			/* The experience of the player            */
	long
	 pk_counter;		/* pk counter list */
};
...
struct char_special_data_saved {
	int
	 alignment;		/* +-1000 for alignments                */
	long
	 idnum;			/* player's idnum; -1 for mobiles   */
...
struct char_special_data {
	...
	int
	 carry_weight;		/* Carried weight */
	int
	 carry_items;		/* Number of items carried   */
	int
	 timer;			/* Timer for update  */

	struct char_special_data_saved
	 saved;			/* constants saved in plrfile  */
};
...
struct mob_special_data {
	...
	int
	 attack_type;		/* The Attack Type Bitvector for NPC's     */
...
struct spell_mem_queue {
	...
	int
	 stored;		// накоплено манны
	int
	 total;			// полное время мема всей очереди
};

/* ================== Structure for player/non-player ===================== */
struct char_data {
	int
	 pfilepos;		/* playerfile pos                */
	mob_rnum nr;		/* Mob's rnum                   */
	room_rnum in_room;	/* Location (real room number)   */
	room_rnum was_in_room;	/* location for linkdead people  */
	int
	 wait;			/* wait for how many loops         */
	int
	 punctual_wait;		/* wait for how many loops (punctual style) */
	char *
	 last_comm;		/* последний приказ чармису перед окончанием лага */

	struct char_player_data
	 player;		/* Normal data                   */
	struct char_played_ability_data
	 add_abils;		/* Abilities that add to main */
	struct char_ability_data
	 real_abils;		/* Abilities without modifiers   */
	struct char_point_data
	 points;		/* Points                       */
	struct char_special_data
	 char_specials;		/* PC/NPC specials     */
	struct player_special_data *player_specials;	/* PC specials        */
	struct mob_special_data
	 mob_specials;		/* NPC specials        */
	...
	struct spell_mem_queue
	 MemQueue;		// очередь изучаемых заклинаний
	struct quest_data
	 Questing;
	struct mob_kill_data
	 MobKill;
