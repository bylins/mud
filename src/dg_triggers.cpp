...
void random_mtrigger(CHAR_DATA * ch)
{
	...
	if (!SCRIPT_CHECK(ch, MTRIG_RANDOM) || AFF_FLAGGED(ch, EAffectFlags::AFF_CHARM))
		return;
...
void speech_mtrigger(CHAR_DATA * actor, char *str)
{
	...
	for (ch = world[IN_ROOM(actor)]->people; ch; ch = ch_next)
	{
		...
		if (SCRIPT_CHECK(ch, MTRIG_SPEECH) && AWAKE(ch) &&
				!AFF_FLAGGED(ch, EAffectFlags::AFF_DEAFNESS) && CAN_START_MTRIG(ch) && (actor != ch))
			for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next)
			{
...
int death_mtrigger(CHAR_DATA * ch, CHAR_DATA * actor)
{
	...
	if (!SCRIPT_CHECK(ch, MTRIG_DEATH) || AFF_FLAGGED(ch, EAffectFlags::AFF_CHARM))
		return 1;
