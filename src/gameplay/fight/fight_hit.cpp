...
int CalcNoparryhitDmg(CharData *ch, ObjData *wielded) {
	if (!skills::GetSkill(ch, ESkill::kNoParryHit)) return 0;
	...
	double skill_mod = static_cast<double>(skills::GetSkill(ch, ESkill::kNoParryHit)) / 5.0;
...
//			weap_skill_is = CalcCurrentSkill(ch, weap_skill, victim);
	...
	if ((skill_num == ESkill::kUndefined || skill_num == ESkill::kBackstab) && skills::GetSkill(ch, ESkill::kNoParryHit)) {
...
void HitData::CalcBaseHitroll(CharData *ch) {
	...
	if (ch->battle_affects.get(kEafOverwhelm) || ch->battle_affects.get(kEafHammer)) {
		calc_thaco -= std::max(0, (skills::GetSkill(ch, weap_skill) - 70) / 8);
	}
	...
	if ((skill_num == ESkill::kThrow
		...
		if (skill_num == ESkill::kBackstab) {
			calc_thaco -= std::max(0, (skills::GetSkill(ch, ESkill::kSneak) + skills::GetSkill(ch, ESkill::kHide) - 100) / 30);
		}
	} else {
		calc_thaco += 4;
	}
...
void HitData::CalcCircumstantialHitroll(CharData *ch, CharData *victim) {
	...
	if (IsAffectedBySpell(ch, ESpell::kCourage)) {
		...
		if (prob > range) {
			dam += ((skills::GetSkill(ch, ESkill::kCourage) + 19) / 20);
			calc_thaco -= static_cast<int>(((skills::GetSkill(ch, ESkill::kCourage) + 9.0) / 20.0) * p_hitroll);
		}
	}
	...
	if (skill_num != ESkill::kThrow && skill_num != ESkill::kBackstab) {
		int prob = (skills::GetSkill(ch, weap_skill) + cha_app[GetRealCha(ch)].illusive) -
			(skills::GetSkill(victim, weap_skill) + int_app[GetRealInt(victim)].observation);

		if (prob >= 30 && !victim->battle_affects.get(kEafAwake)
			&& (ch->IsNpc() || !ch->battle_affects.get(kEafPunctual))) {
			calc_thaco -= static_cast<int>((skills::GetSkill(ch, weap_skill) -
				skills::GetSkill(victim, weap_skill) > 60 ? 2 : 1) * p_hitroll);
			if (!victim->IsNpc())
...
void HitData::CalcStaticHitroll(CharData *ch) {
	...
	if (weapon == fight::AttackType::kOffHand
		...
		&& !(wielded && wielded->get_type() == EObjType::kWeapon)
		&& !ch->IsNpc()) {
		calc_thaco += (MUD::Skill(ESkill::kLeftHit).difficulty - skills::GetSkill(ch, ESkill::kLeftHit)) / 10;
	}
	...
	// скилл владения пушкой или голыми руками
	if (skills::GetSkill(ch, weap_skill) <= 80)
		calc_thaco -= static_cast<int>((skills::GetSkill(ch, weap_skill) / 20.0) * p_hitroll);
	else if (skills::GetSkill(ch, weap_skill) <= 160)
		calc_thaco -= static_cast<int>((4.0 + (skills::GetSkill(ch, weap_skill) - 80.0) / 10.0) * p_hitroll);
	else
		calc_thaco -= static_cast<int>((12.0 + (skills::GetSkill(ch, weap_skill) - 160.0) / 5.0) * p_hitroll);
}
...
void HitData::CalcCritHitChance(CharData *ch) {
	...
	if ((!ch->IsNpc() && !IsMage(ch) && !IS_MAGUS(ch))
		...
		calc_critic = std::min(skills::GetSkill(ch, weap_skill), 70);
		if (CanUseFeat(ch, FindWeaponMasterFeat(weap_skill))) {
			calc_critic += std::max(0, skills::GetSkill(ch, weap_skill) - 70);
		}
		if (CanUseFeat(ch, EFeat::kThieveStrike)) {
			calc_critic += skills::GetSkill(ch, ESkill::kBackstab);
		}
		if (!ch->IsNpc()) {
			calc_critic += static_cast<int>(skills::GetSkill(ch, ESkill::kPunctual) / 2);
			calc_critic += static_cast<int>(skills::GetSkill(ch, ESkill::kNoParryHit) / 3);
		}
...
int HitData::CalcDmg(CharData *ch, bool need_dice) {
	...
	if (skills::GetSkill(ch, weap_skill) == 0) {
	...
	if (skills::GetSkill(ch, weap_skill) >= 60) { //от уровня скилла
		dam += ((skills::GetSkill(ch, weap_skill) - 50) / 10);
		if (ch->IsFlagged(EPrf::kExecutor))
	...
	if (skill_num < ESkill::kFirst) {
		dam += GetAutoattackDamroll(ch, skills::GetSkill(ch, weap_skill));
	if (ch->IsFlagged(EPrf::kExecutor))
	...
	if (ch->battle_affects.get(kEafIronWind)) {
		dam += skills::GetSkill(ch, ESkill::kIronwind) / 2;
		if (ch->IsFlagged(EPrf::kExecutor))
...
//	if (CanUseFeat(ch, EFeat::kBowsFocus) && skills::GetSkill(ch, ESkill::kAddshot)) {
//		bonus *= 3;
//	}
	return bonus;
}
