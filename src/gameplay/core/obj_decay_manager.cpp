...
void ObjDecayManager::track_timed_spell(ObjData *obj) {
	if (obj) {
		m_timed_spell_index.insert(obj);
	}
}

void ObjDecayManager::untrack_timed_spell(ObjData *obj) {
	m_timed_spell_index.erase(obj);
}
...
TickResult ObjDecayManager::process_tick() {
	...
	{
		auto ts_copy = m_timed_spell_index;
