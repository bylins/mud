...
*/
	...
	int skill_modi =std::max(0, skills::GetSkill(ch, ESkill::kKick) * 2 / 5);
	...
	dam = number(dam * 2  /5, dam);
		if (mount::IsOnHorse(ch) && (skills::GetSkill(ch, ESkill::kRiding) >= 150) && (skills::GetSkill(ch, ESkill::kKick) >= 150)) {
			Affect<EApply> af;
			af.location = EApply::kNone;
			af.type = ESpell::kBattle;
			af.modifier = 0;
			af.battleflag = 0;
			float modi = ((skills::GetSkill(ch, ESkill::kKick) + GetRealStr(ch) * 5)
				+ (GET_EQ(ch, EEquipPos::kFeet) ? GET_EQ(ch, EEquipPos::kFeet)->get_weight() : 0) * 3) / float(GET_SIZE(vict));
			if (number(1, 1000) < modi * 10) {
				switch (number(0, (skills::GetSkill(ch, ESkill::kKick) - 150) / 10)) {
					case 0:
					case 1:
			...
			} else if (number(1, 1000) < (skills::GetSkill(ch, ESkill::kRiding) / 2)) {
				dam *= 2;
				if (!ch->IsNpc())
...
void do_kick(CharData *ch, char *argument, int/* cmd*/, int/* subcmd*/) {
	if (skills::GetSkill(ch, ESkill::kKick) < 1) {
