...
int CalcLeadership(CharData *ch) {
	...
	if (!skills::GetSkill(leader, ESkill::kLeadership)) {
		return (false);
	}
...
//		bonus_commander = VPOSI((ch->get_skill(ESkill::kLeadership) - 120) / 10, 0, 8);
	auto bonus_commander = VPOSI((skills::GetSkill(leader, ESkill::kLeadership) - 200) / 8, 0, 8);
	return static_cast<int>(VPOSI((skills::GetSkill(leader, ESkill::kLeadership) - 80) / 5, 0, 4) + bonus_commander);
}
...
void UpdateLeadership(CharData *ch, CharData *killer) {
	...
	if (ch->IsNpc() && killer) {
		if (!killer->IsNpc() // Убил загрупленный чар
			...
			&& killer->has_master()
			&& skills::GetSkill(killer->get_master(), ESkill::kLeadership) > 0
			...
			if (killer->get_master()->has_master() // Владелец чармиса НЕ лидер
				&& skills::GetSkill(killer->get_master()->get_master(), ESkill::kLeadership) > 0
				&& killer->in_room == killer->get_master()->in_room
	...
	if (!ch->IsNpc() // Член группы убит мобом
		...
		&& skills::GetTrainedSkill(ch->get_master(), ESkill::kLeadership) > 1) {
		const auto current_skill = skills::GetSkillBonus(ch->get_master(), ESkill::kLeadership);
		skills::SetSkill(ch->get_master(), ESkill::kLeadership, current_skill - 1);
	}
}
