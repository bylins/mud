...
void DoFirstaid(CharData *ch, char *argument, int/* cmd*/, int/* subcmd*/) {
	...
	if (IS_IMMORTAL(ch) || (IS_SET(ch->player_specials->saved.GodsLike, EGf::kGodsLike)) || (IS_SET(vict->player_specials->saved.GodsLike, EGf::kGodsLike))) {
		percent = 0;
	}
	if ((IS_SET(ch->player_specials->saved.GodsLike, EGf::kGodscurse)) || (IS_SET(vict->player_specials->saved.GodsLike, EGf::kGodscurse))) {
		prob = 0;
	}
