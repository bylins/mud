...
// issue.magic-items: potion-style maker competence for scrolls/wands/staves (reads the kSpellItem*
// keys). SpellItemPotency = C for a given spell; SpellItemSkill = the maker skill for buff duration.
[[nodiscard]] float SpellItemPotency(const ObjData *item, ESpell spell_id);
[[nodiscard]] int SpellItemSkill(const ObjData *item);
