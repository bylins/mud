...
// issue.runestones: the runestone mechanics, extracted from townportal.{h,cpp}. A runestone is a
// readable stone that sits in a room; a character who reads (examines) one with enough skill
// memorises it and can later open a town-portal to that room. The townportal *skill* and the
// one-way-portal timer stay in townportal.{h,cpp} (they use these).
...
class RunestoneRoster : private std::vector<Runestone> {
 ...
 class CharacterRunestoneRoster : private std::vector<RoomVnum> {
  public:
   ...
   // issue.runestones: the player-facing add/remove (with messages) lived in CharData; moved here.
