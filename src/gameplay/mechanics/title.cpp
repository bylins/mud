...
const char *GOD_DO_TITLE_FORMAT =
	...
	"титул удалить - удалить свой текущий титул и предтитул (писать полную команду)\r\n";
const char
	*TITLE_SEND_FORMAT = "Вы можете бесплатно изменить ее, отправив новую, или отменить командой 'титул отменить'\r\n";
...
bool check_pre_title(const std::string& text, CharData *ch);
...
void TitleSystem::do_title(CharData *ch, char *argument, int/* cmd*/, int/* subcmd*/) {
	...
	if (CompareParam(buffer2, "установить")) {
		...
		if (it != temp_title_list.end()) {
			if (currencies::GetAmount(*ch, currencies::kKunaId, currencies::EPurse::kBank) < SET_TITLE_COST) {
				SendMsgToChar("На вашем счету не хватает денег для оплаты этой услуги.\r\n", ch);
				return;
			}
			currencies::RemoveAmount(*ch, currencies::kKunaId, SET_TITLE_COST, currencies::EPurse::kBank);
		...
		if (it != title_list.end()) {
			title_list.erase(it);
			currencies::AddAmount(*ch, currencies::kKunaId, SET_TITLE_COST, currencies::EPurse::kBank);
