...
void ApplyRiderToHit(CharData *ch, CharData *victim, int &calc_thaco) {
	...
	calc_thaco += 10 - skills::GetSkill(ch, ESkill::kRiding) / 20;
}

void ApplyRiderHitAndDamage(CharData *ch, int &dam, int &calc_thaco) {
	const int prob = skills::GetSkill(ch, ESkill::kRiding);
...
void ApplyRiderDamageMult(CharData *ch, int &dam) {
	if (IsOnHorse(ch) && skills::GetSkill(ch, ESkill::kRiding) > 100) {
		dam *= 1.0 + (skills::GetSkill(ch, ESkill::kRiding) - 100) / 500.0;
...
int SavingModifier(const CharData *ch, ESaving saving) {
	...
	switch (saving) {
		...
		case ESaving::kStability: return -20 - skills::GetSkill(ch, ESkill::kRiding) / 25;
		default: return 0;
	}
}
