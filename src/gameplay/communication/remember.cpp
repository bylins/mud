...
void do_remember_char(CharData *ch, char *argument, int/* cmd*/, int/* subcmd*/) {
	...
	} else if (utils::IsAbbr(arg, "группа") || utils::IsAbbr(arg, "ггруппа"))
	{
		...
		if (ch->player_specials->clan) {
			SendMsgToChar(ch->player_specials->clan->get_remember(ch->remember_get_num(), Remember::CLAN), ch);
		} else {
			SendMsgToChar(ch, "Вам нечего вспомнить.\r\n");
		}
		return;
		...
		if (ch->player_specials->clan) {
			SendMsgToChar(ch->player_specials->clan->get_remember(ch->remember_get_num(), Remember::ALLY), ch);
		} else {
			SendMsgToChar(ch, "Вам нечего вспомнить.\r\n");
		}
		return;
