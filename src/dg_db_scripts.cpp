...
void trg_featturn(CHAR_DATA * ch, int featnum, int featdiff)
{
	...
	if (HAVE_FEAT(ch, featnum))
	{
		...
		send_to_char(ch, "Вы утратили способность '%s'.\r\n", feat_info[featnum].name);
		log("Remove %s from %s (trigfeat)", feat_info[featnum].name, GET_NAME(ch));
	}
	else if (feat_info[featnum].classknow[(int) GET_CLASS(ch)][(int) GET_KIN(ch)])
	{
		SET_FEAT(ch, featnum);
		send_to_char(ch, "Вы обрели способность '%s'.\r\n", feat_info[featnum].name);
		log("Add %s to %s (trigfeat)", feat_info[featnum].name, GET_NAME(ch));
	}
}
...
void trg_skillturn(CHAR_DATA * ch, int skillnum, int skilldiff)
{
	...
	else if (skill_info[skillnum].classknow[ch_kin][ch_class] == KNOW_SKILL
