...
//      GET_NAME(victim), GET_NAME(ch), modi, spellnum);
	...
	case SPELL_SHOCK:
		...
		case SPELL_WC_OF_THUNDER:
			...
			if ((IS_NPC(victim) && AFF_FLAGGED(victim, static_cast<int>(af[0].bitvector))) ||
...
//Polud пробный обработчик аффектов
	...
	if (IS_NPC(victim) && success)
		for (i = 0; i < MAX_SPELL_AFFECTS && success; i++)
			if (AFF_FLAGGED(victim, static_cast<int>(af[i].bitvector)))
			{
				if (IN_ROOM(ch) == IN_ROOM(victim))
					send_to_char(NOEFFECT, ch);
				success = FALSE;
...
//added by Adept
	if (spellnum == SPELL_SUMMON_FIREKEEPER)
	{
		...
		{
			AFF_FLAGS(mob).set(AFF_FIRESHIELD);
		}
		else
		{
			AFF_FLAGS(mob).set(AFF_FIREAURA);
		}
...
int mag_single_target(int level, CHAR_DATA * caster, CHAR_DATA * cvict, OBJ_DATA * ovict, int spellnum, int savetype)
{
	...
	if ((cvict && IS_GOD(cvict)) || (IS_NPC(caster) || ((GET_LEVEL(cvict) / 2) > GET_LEVEL(caster)))) // при разнице уровня в 2 раза закл фэйл (из-за опечатки не работало)
	{
