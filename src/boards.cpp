...
void Board::BoardInit()
{
	...
	GeneralBoard->desc = "Основной раздел";
	...
	GodBuildBoard->desc = "Раздел для ваятелей зон";
	...
	GodCodeBoard->desc = "Раздел для ваятелей кода";
...
void Board::ClanInit()
{
	...
	for (ClanListType::const_iterator clan = Clan::ClanList.begin(); clan != Clan::ClanList.end(); ++clan) {
		...
		board->name = "ДрВече";
		board->desc = "Основной раздел Дружины " + (*clan)->abbrev;
		...
		newsboard->desc = "Новости Дружины " + (*clan)->abbrev;
...
ACMD(DoBoard)
{
	...
	if (CompareParam(buffer, "список") || CompareParam(buffer, "list")) {
		...
		for (MessageListType::reverse_iterator message = (*board)->messages.rbegin(); message != (*board)->messages.rend(); ++message) {
			if ((*message)->date > date) {
				// для выделения новых мессаг светло-белым
				body << CCWHT(ch, C_NRM);
			}
			strftime(timeBuf, sizeof(timeBuf), "%H:%M %d-%m-%Y", localtime(&(*message)->date));
	...
	if (CompareParam(buffer, "очистить") || CompareParam(buffer, "remove")) {
		...
		num = (*board)->messages.size() - num;
		(*board)->SetLastReadDate(ch, (*board)->messages[num]->date);
		// или он может делетить любые мессаги (по левелу/рангу потом еще смотрим), или только свои
		...
		// для клановых полный доступ только у воеводы, у персональных смысла нет
				send_to_char("У Вас нет возможности удалить это сообщение.\r\n", ch);
				return;
		}
...
SPECIAL(Board::Special)
{
	...
	if (CMD_IS("смотреть") || CMD_IS("осмотреть") || CMD_IS("look") || CMD_IS("examine")
			|| ((CMD_IS("читать") || CMD_IS("read")) && isname(buffer2.c_str(), board->name)) ) {
	...
	if (CMD_IS("читать") || CMD_IS("read") || CMD_IS("писать") || CMD_IS("write")
		|| CMD_IS("очистить") || CMD_IS("remove")) {
		// общая доска
