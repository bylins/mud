...
int exchange_exhibit(CharData *ch, char *arg) {
	...
	for (j = exchange_item_list, counter = 0, counter_ming = 0;
		 ...
		 j = next_thing) {
		next_thing = j->next;
		if (GET_EXCHANGE_ITEM_SELLERID(j) == GET_ID(ch)) {
	...
	GET_EXCHANGE_ITEM_SELLERID(item) = GET_ID(ch);
...
int exchange_change_cost(CharData *ch, char *arg) {
	...
	if ((GET_EXCHANGE_ITEM_SELLERID(item) != GET_ID(ch)) && (GetRealLevel(ch) < kLvlImplementator)) {
...
int exchange_withdraw(CharData *ch, char *arg) {
	...
	if (GET_EXCHANGE_ITEM_SELLERID(item) != GET_ID(ch)) {
