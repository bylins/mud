...
void showlots(CharData *ch) {
	...
	for (int i = 0; i < kMaxAuctionLot; i++) {
		...
		if (GET_LOT(i)->prefect && GET_LOT(i)->prefect_unique == GET_ID(ch)) {
...
*/
		...
		case 1:        // Close
			...
			if (GET_LOT(lot)->seller != ch || GET_LOT(lot)->seller_unique != GET_ID(ch)) {
		...
		case 2:        // Set
			...
			if (GET_LOT(lot)->prefect && GET_LOT(lot)->prefect_unique > 0 &&
				(GET_LOT(lot)->prefect != ch || GET_LOT(lot)->prefect_unique != GET_ID(ch))) {
			...
			GET_LOT(lot)->buyer_unique = GET_ID(ch);
		...
		case 4:        // Transport //
			...
			if (GET_LOT(lot)->prefect != ch || GET_LOT(lot)->prefect_unique != GET_ID(ch)) {
...
int check_sell(int lot) {
	...
	if (lot < 0 || lot >= kMaxAuctionLot || !(ch = GET_LOT(lot)->seller)
		|| GET_ID(ch) != GET_LOT(lot)->seller_unique || !(tch = GET_LOT(lot)->buyer)
		|| GET_ID(tch) != GET_LOT(lot)->buyer_unique || !(obj = GET_LOT(lot)->item)
	...
	if (ch) {
		for (i = 0; i < kMaxAuctionLot; i++) {
			if (!GET_LOT(i)->seller || !GET_LOT(i)->item)
				continue;
			if (GET_LOT(i)->seller == ch || GET_LOT(i)->seller_unique == GET_ID(ch)
				|| GET_LOT(i)->buyer == ch || GET_LOT(i)->buyer_unique == GET_ID(ch)
				|| GET_LOT(i)->prefect == ch || GET_LOT(i)->prefect_unique == GET_ID(ch)) {
