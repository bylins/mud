...
//5
	initSpell(SPELL_BURNING_HANDS, "горящие руки", "burning hands", 40, 30,
			  1, POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, MTYPE_AGGRESSIVE,
			  MAG_AREAS | MAG_DAMAGE | NPC_DAMAGE_PC, 1, STYPE_FIRE);
//6
...
//10
	initSpell(SPELL_COLOR_SPRAY, "ледяные стрелы", "ice bolts", 90, 75, 1,
...
//15
	initSpell(SPELL_CURE_CRITIC, "критическое исцеление", "critical cure",
			  100, 90, 1, POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_SELF, false,
			  MAG_POINTS | NPC_DUMMY, 3, STYPE_LIFE);
//16
	initSpell(SPELL_CURE_LIGHT, "легкое исцеление", "light cure",
			  40, 30, 1, POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_SELF, false,
...
//23
	initSpell(SPELL_EARTHQUAKE, "землетрясение", "earthquake", 110, 90, 2,
			  POS_FIGHTING, TAR_IGNORE, MTYPE_AGGRESSIVE, MAG_MASSES | MAG_DAMAGE | NPC_DAMAGE_PC,
			  2, STYPE_EARTH);
//24
	initSpell(SPELL_ENCHANT_WEAPON, "заколдовать оружие", "enchant weapon",
			  140, 110, 2, POS_STANDING, TAR_OBJ_INV, false, MAG_ALTER_OBJS,
			  0, STYPE_LIGHT);
//25
...
//40
	initSpell(SPELL_SUMMON, "призвать", "summon",
			  110, 100, 2, POS_STANDING, TAR_CHAR_WORLD | TAR_NOT_SELF, false,
			  MAG_MANUAL, 0, STYPE_MIND);
//41
	initSpell(SPELL_PATRONAGE, "покровительство", "patronage", 85, 70, 2,
			  POS_FIGHTING, TAR_SELF_ONLY | TAR_CHAR_ROOM, false, MAG_POINTS | MAG_AFFECTS,
			  1, STYPE_LIGHT);
//42
