...
//3
	feato(BLIND_FIGHT_FEAT, "слепой боец", NORMAL_FTYPE, TRUE, feat_app);
//4
...
//18
	feat_app.insert(SKILL_DRUNKOFF, 20);
	feat_app.insert(FEAT_TIMER, 6);
...
//22
	...
	feat_app.clear();
//23
	feat_app.insert(SKILL_HIDE, 5);
	feat_app.insert(SKILL_SNEAK, 5);
	feat_app.insert(SKILL_CAMOUFLAGE, 5);
...
//25 -*
	...
	feat_app.clear();
//26
	feat_app.insert(SKILL_TRACK, 10);
	feat_app.insert(SKILL_SENSE, 10);
	feato(TRACKER_FEAT, "следопыт", SKILL_MOD_FTYPE, TRUE, feat_app);
	feat_app.clear();
/*
//27
	feato(WEAPON_FINESSE_FEAT, "фехтование", NORMAL_FTYPE, TRUE, feat_app);
//28
...
//33
	feato(SHORTS_MASTER_FEAT, "мастер кинжала", NORMAL_FTYPE, TRUE, feat_app);
//34
...
//36
	feato(PICK_MASTER_FEAT, "мастер ножа", NORMAL_FTYPE, TRUE, feat_app);
//37
...
//53
*/
	feat_app.insert(SKILL_PICK_LOCK, 5);
	feat_app.insert(SKILL_STEAL, 5);
...
bool can_get_feat(CHAR_DATA *ch, int feat)
{
	if (feat <= 0 || feat >= MAX_FEATS) {
		sprintf(buf, "Неверный номер способности передан в features::can_get_feat - %d!", feat);
