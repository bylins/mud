...
void Dps::add_dmg(int type, CharData *ch, int dmg, int over_dmg) {
	switch (type) {
...
void Dps::end_round(int type, CharData *ch) {
	switch (type) {
...
void Dps::add_tmp_group_list(CharData *ch) {
...
*/
void Dps::print_stats(CharData *ch, CharData *coder) {
	if (!coder) {
		coder = ch;
	}
...
* \param coder - случай вывода статистики другому персонажу, по умолчанию = 0
*/
void Dps::print_group_stats(CharData *ch, CharData *coder) {
	if (!coder) {
		coder = ch;
	}
...
void Dps::add_group_dmg(CharData *ch, int dmg, int over_dmg) {
	GroupListType::iterator it = group_dps_.find(GET_ID(ch));
	if (it != group_dps_.end()) {
...
void Dps::end_group_round(CharData *ch) {
	GroupListType::iterator it = group_dps_.find(GET_ID(ch));
	if (it != group_dps_.end()) {
		it->second.end_round();
	} else {
...
void Dps::add_group_charm_dmg(CharData *ch, int dmg, int over_dmg) {
...
void Dps::end_group_charm_round(CharData *ch) {
	GroupListType::iterator it = group_dps_.find(GET_ID(ch->get_master()));
	if (it != group_dps_.end()) {
