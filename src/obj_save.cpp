		...
		//if (read_line != NULL) read_line cannot be NULL because it is a local array
		{
			...
			} else if (!strcmp(read_line, "ClID")) // id чара
			{
				*error = 61;
				...
				object->get_custom_label()->AuthorIdnum = atoi(buffer);
				if (object->get_custom_label()->AuthorIdnum > 0) {
					for (std::size_t i = 0; i < player_table.size(); i++) {
						if (player_table[i].id() == object->get_custom_label()->AuthorIdnum) {
...
//			&& object->get_manual_mort_req() != proto->get_manual_mort_req())
	...
	if (object->get_custom_label()) {
		...
		out << "ClID: " << object->get_custom_label()->AuthorIdnum << "~\n";
