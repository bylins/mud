...
*/
		...
		// Временный обкаст (яд на оружии и прочее) стареет сам по себе и к правилам живучести
		// предмета отношения не имеет: вещь может быть с бесконечным таймером или с !рента-таймер,
		// но нанесенный на нее яд все равно выдыхается по часам. Раньше это делалось заодно внутри
		// dec_timer, то есть под чужими гардами -- теперь заказываем явно и для всех.
		obj->dec_timed_spell(timer_dec);
