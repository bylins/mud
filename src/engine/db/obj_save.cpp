...
*/
		...
		// issue #3678: временный обкаст (яд на оружии и прочее) в ренте намеренно НЕ стареет --
		// ровно как заклинания и кличи на самом персонаже, которые тоже замирают до следующего
		// входа. Раньше он старел заодно с живучестью, потому что сидел внутри dec_timer: игрок
		// платил за ночь в ренте выдохшимся ядом, хотя баф на нем самом дожидался его целым.
		// Живучесть предмета при этом стареет как и раньше -- рента для того и место, где вещи
		// ветшают. Если понадобится вернуть старение обкаста, тут зовется dec_timed_spell().
