...
void HitData::compute_critical(CharData *ch, CharData *victim) {
		...
		case 4:        // FEETS
			switch (dam_critic) {
				...
				case 11:    // femor damaged, no speed, no attack
					...
					af[0].type = ESpell::kBattle;
					af[0].affect_bits = to_underlying(EAffect::kStopFight);
			...
			switch (dam_critic) {
				...
				case 10:    // hand damaged, no attack this
					if (!AFF_FLAGGED(victim, EAffect::kStopRight)) {
						...
						af[0].type = ESpell::kBattle;
						af[0].affect_bits = to_underlying(EAffect::kStopRight);
						af[0].duration = CalcDuration(victim, 30, 0, 0, 0, 0);
						af[0].flags = kAfBattledec | kAfPulsedec;
						...
						af[0].type = ESpell::kBattle;
						af[0].affect_bits = to_underlying(EAffect::kStopLeft);
						af[0].duration = CalcDuration(victim, 30, 0, 0, 0, 0);
						af[0].flags = kAfBattledec | kAfPulsedec;
					} else {
						to_char = "вывело $N3 из строя";
	...
	for (int i = 0; i < 4; i++) {
		if (af[i].type > ESpell::kUndefined) {
			if (af[i].affect_bits == to_underlying(EAffect::kStopFight)
				|| af[i].affect_bits == to_underlying(EAffect::kStopRight)
				|| af[i].affect_bits == to_underlying(EAffect::kStopLeft)) {
