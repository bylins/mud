...
class CCraft {
 ...
 private:
	...
	std::set<id_t> m_material;              ///< List of materials used by this crafts.
...
class CCraftModel {
 public:
	...
	/**
	 * Merges loaded crafts model into MUD world.
	 */
	bool merge();
 ...
 private:
	...
	*/
	...
	prototypes_t m_prototypes;    ///< List of objects defined by the crafts system.
	materials_t m_materials;    ///< List of materials

	// Properties
	int m_base_count;                        ///< Base count of crafts (per character?).
	int m_remort_for_count_bonus;            ///< Required remorts to take one more crafts.
	...
	std::list<std::string> m_craft_files;        ///< List of files with crafts definitions.
