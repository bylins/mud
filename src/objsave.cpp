...
enum
{
	RENT_UNDEF,    // ÎÅ ÉÓÐÏÌØÚÕÅÔÓÑ
	RENT_CRASH,    // ÒÅÇÕÌÑÒÎÙÊ Á×ÔÏÓÅÊ× ÎÁ ÓÌÕÞÁÊ ËÒÅÛÁ
	RENT_RENTED,   // ÚÁÒÅÎÔÉÌÓÑ ÓÁÍ
	RENT_CRYO,     // ÎÅ ÉÓÐÏÌØÚÕÅÔÓÑ
	RENT_FORCED,   // ÚÁÒÅÎÞÅÎ ÎÁ ÒÅÂÕÔÅ
	RENT_TIMEDOUT, // ÚÁÒÅÎÞÅÎ ÐÏÓÌÅ idle_rent_time
};
...
// äÁÎÎÁÑ ÐÒÏÃÅÄÕÒÁ ×ÙÂÉÒÁÅÔ ÐÒÅÄÍÅÔ ÉÚ ÂÕÆÅÒÁ.
// Ó ÐÏÄÄÅÒÖËÏÊ ÎÏ×ÏÇÏ ÆÏÒÍÁÔÁ ×ÅÝÅÊ ÉÇÒÏËÏ× [ÏÔ 10.12.04].
OBJ_DATA::shared_ptr read_one_object_new(char **data, int *error)
{
	...
	*error = 3;
	...
	// åÓÌÉ ÂÅÚ ÐÒÏÔÏÔÉÐÁ, ÓÏÚÄÁÅÍ ÎÏ×ÙÊ. éÎÁÞÅ ÞÉÔÁÅÍ ÐÒÏÔÏÔÉÐ É ×ÏÚ×ÒÁÝÁÅÍ NULL,
	// ÅÓÌÉ ÐÒÏÔÏÔÉÐ ÎÅ ÓÕÝÅÓÔ×ÕÅÔ.
	if (vnum < 0)
	{
	...
	// äÁÌÅÅ ÎÁÊÄÅÎÎÙÅ ÐÁÒÁÍÅÔÒÙ ÐÒÉÓ×ÁÉ×ÁÅÍ Ë ÐÒÏÔÏÔÉÐÕ
	while (get_buf_lines(data, buffer))
	{
		...
		{
			// þÔÏÂÙ ÎÅ ÐÏÒÔÉÌÓÑ ÐÒÏÔÏÔÉÐ ×ÅÝÉ ÎÅËÏÔÏÒÙÅ ÐÁÒÁÍÅÔÒÙ ÉÚÍÅÎÑÀÔÓÑ ÌÉÛØ Õ ×ÅÝÅÊ Ó vnum < 0
			...
			else if (!strcmp(read_line, "Clbl")) // ÔÅËÓÔ ÍÅÔËÉ
			{
				*error = 60;
			...
			else if (!strcmp(read_line, "ClID")) // id ÞÁÒÁ
			{
				*error = 61;
				if (!object->get_custom_label())
				{
			...
			else if (!strcmp(read_line, "ClCl")) // ÁÂÂÒÅ×ÉÁÔÕÒÁ ËÌÁÎÁ
			{
				*error = 62;
				if (!object->get_custom_label())
				{
	...
	*error = 0;
	...
	if (object->get_extra_flag(EExtraFlag::ITEM_NAMED))//éÍÅÎÎÏÊ ÐÒÅÄÍÅÔ
	{
...
// äÁÎÎÁÑ ÐÒÏÃÅÄÕÒÁ ×ÙÂÉÒÁÅÔ ÐÒÅÄÍÅÔ ÉÚ ÂÕÆÅÒÁ
// ÷îéíáîéå!!! ÜÔÁ ÆÕÎËÃÉÑ ÉÓÐÏÌØÚÕÅÔÓÑ ÔÏÌØËÏ ÄÌÑ ÞÔÅÎÉÑ ×ÅÝÅÊ ÐÅÒÓÏÎÁÖÁ,
// ÓÏÈÒÁÎÅÎÎÙÈ × ÓÔÁÒÏÍ ÆÏÒÍÁÔÅ, ÄÌÑ ÞÔÅÎÉÑ ÎÏ×ÏÇÏ ÆÏÒÍÁÔÁ ÐÒÉÍÅÎÑÅÔÓÑ Æ-ÉÑ read_one_object_new
OBJ_DATA::shared_ptr read_one_object(char **data, int *error)
{
	...
	*error = 3;
	...
	OBJ_DATA::shared_ptr object;
	if (vnum < 0)  		// ðÒÅÄÍÅÔ ÎÅ ÉÍÅÅÔ ÐÒÏÔÏÔÉÐÁ
	{
		object = world_objects.create_blank();
		*error = 4;
		if (!get_buf_lines(data, buffer))
	...
	*error = 15;
	...
	for (;;)
	{
		...
		case 'M':
			// ÷ÓÔÁ×ÌÑÅÍ ÓÀÄÁ ÕÎÉËÁÌØÎÙÊ ÎÏÍÅÒ ÓÏÚÄÁÔÅÌÑ
			if (!get_buf_line(data, buffer))
			{
				*error = 20;
				return object;
...
// shapirus: ÆÕÎËÃÉÑ ÐÒÏ×ÅÒËÉ ÎÁÌÉÞÉÑ ÄÏÐ. ÏÐÉÓÁÎÉÑ × ÐÒÏÔÏÔÉÐÅ
...
// äÁÎÎÁÑ ÐÒÏÃÅÄÕÒÁ ÐÏÍÅÝÁÅÔ ÐÒÅÄÍÅÔ × ÂÕÆÅÒ
// [ éóðïìøúõåôóñ ÷ îï÷ïí æïòíáôå ÷åýåê ðåòóïîáöá ïô 10.12.04 ]
void write_one_object(std::stringstream &out, OBJ_DATA * object, int location)
{
	...
	// ðÏÌÏÖÅÎÉÅ × ÜËÉÐÉÒÏ×ËÅ (ÓÏÈÒÁÎÑÅÍ ÔÏÌØËÏ ÅÓÌÉ > 0)
	if (location)
	{
		out << "Lctn: " << location << "~\n";
	}

	// åÓÌÉ Õ ÛÍÏÔËÉ ÅÓÔØ ÐÒÏÔÏÔÉÐ ÔÏ ÂÕÄÅÍ ÓÏÈÒÁÎÑÔØ ÐÏ ÏÂÒÅÚÁÎÎÏÊ ÓÈÅÍÅ, ÉÎÁÞÅ
	// ÐÒÉÄÅÔÓÑ ÓÏÈÒÁÎÑÔØ ×ÓÅ ÓÔÁÔÓÙ ÛÍÏÔËÉ.
  ...
  auto obj_ptr = world_objects.get_by_raw_ptr(object);
	...
	if (GET_OBJ_VNUM(object) >= 0 && proto)
	{
		...
		if (object->get_timer() != proto->get_timer())
		{
			...
			if (!check_obj_in_system_zone(GET_OBJ_VNUM(object))) //ÅÓÌÉ ÛÍÏÔ × ÓÌÕÖÅÂÎÏÊ ÚÏÎÅ, ÔÏ ÔÁÊÍÅÒ ÎÅ ÔÒÏÇÁÅÍ
			{
				// ÎÁ ÔÏÔ ÓÌÕÞÁÊ, ÅÓÌÉ ÅÓÔØ ÏÂßÅËÔÙ Ó ÔÁÊÍÅÒÏÍ, ÂÏÌØÛÅ ÔÁÊÍÅÒÁ ÐÒÏÔÏÔÉÐÁ
		...
		*buf = '\0';
		*buf2 = '\0';
		//÷ÒÅÍÅÎÎÏ ÕÂÉÒÁÅÍ ÆÌÁÇ !ÏËÒÏ×Á×ÌÅÎ! Ó ×ÅÝÉ, ÞÔÏÂÙ ÏÎ ÎÅ ÓÏÈÒÁÎÑÌÓÑ
		...
		if (blooded) //÷ÏÚ×ÒÁÝÁÅÍ ÆÌÁÇ ÎÁÚÁÄ
		{
		...
		*buf2 = '\0';
		...
		// úÎÁÞÅÎÉÅ 0, úÎÁÞÅÎÉÅ 1, úÎÁÞÅÎÉÅ 2, úÎÁÞÅÎÉÅ 3.
		for (i = 0; i < 4; i++)
		{
		...
		// äÏÐ ÏÐÉÓÁÎÉÑ
		// shapirus: ÉÓÐÒÁ×ÌÅÎÁ ÏÛÉÂËÁ Ó ÎÅÓÏÈÒÁÎÅÎÉÅÍ, ÎÁÐÒÉÍÅÒ, ÍÅÔÏË
		// ÎÁ ËÒÁÆÔÅÎÙÈ ÌÕËÁÈ
		...
		// åÓÌÉ Õ ÐÒÏÔÏÔÉÐÁ ÅÓÔØ ÏÐÉÓÁÎÉÅ, Á Õ ÓÏÈÒÁÎÑÅÍÏÊ ×ÅÝÉ - ÎÅÔ, ÓÏÈÒÁÎÑÅÍ None
...
//			&& object->get_manual_mort_req() != proto->get_manual_mort_req())
		...
		// ObjectValue ÐÒÅÄÍÅÔÁ, ÅÓÌÉ ÅÓÔØ ÞÔÏ ÓÏÈÒÁÎÑÔØ
		if (object->get_all_values() != proto->get_all_values())
		{
			out << object->serialize_values();
		}
	}
	else  		// åÓÌÉ Õ ÛÍÏÔËÉ ÎÅÔ ÐÒÏÔÏÔÉÐÁ - ÐÒÉÄÅÔÓÑ ÓÏÈÒÁÎÑÔØ ÅÅ ÃÅÌÉËÏÍ.
	{
		// áÌÉÁÓÙ
	...
	// ÏÂËÁÓÔ (ÅÓÌÉ ÏÎ ÅÓÔØ) ÓÏÈÒÁÎÑÅÔÓÑ × ÌÀÂÏÍ ÓÌÕÞÁÅ ÎÅÚÁ×ÉÓÉÍÏ ÏÔ ÐÒÏÔÏÔÉÐÁ
	if (object->has_timed_spell())
	{
		out << object->timed_spell().print();
	}

	// ÎÁËÌÁÄÙ×ÁÅÍÙÅ ÜÎÞÁÎÔÙ
...
int Crash_read_timer(const std::size_t index, int temp)
{
	...
	case RENT_RENTED:
		...
		break;
	case RENT_CRASH:
		//           if (rent.time<1001651000L) //ËÒÅÛ-ÓÅÊ× ÄÏ Sep 28 00:26:20 2001
		rent.net_cost_per_diem = 0;	//ÂÅÓÐÌÁÔÎÏ!
		strcat(buf, " Crash ");
		break;
	case RENT_CRYO:
		strcat(buf, " Cryo ");
...
void Crash_timer_obj(const std::size_t index, long time)
{
	...
	//ÕÄÁÌÑÅÍ ÐÒÏÓÒÏÞÅÎÎÙÅ ÆÁÊÌÙ ÒÅÎÔÙ
...
//  log("Objects (%d), Deleted (%d)+(%d).", nitems, ideleted, idelete);
	...
	//ÅÓÌÉ ÐÏÑ×ÉÌÉÓØ ÎÏ×ÙÅ ÐÒÏÓÒÏÞÅÎÎÙÅ ÏÂßÅËÔÙ, ÏÂÎÏ×ÌÑÅÍ ÆÁÊÌ ÔÁÊÍÅÒÏ×
	if (idelete)
	{
...
void Crash_list_objects(CHAR_DATA * ch, int index)
{
	...
	for (; i < SAVEINFO(index)->rent.nitems; i++)
	{
		...
		else if ((data.vnum > 799 ) || (data.vnum < 700 ))
		{
			sprintf(buf + strlen(buf), " [%5d] (?????au) <%2d> %-20s\r\n",
				data.vnum, MAX(-1, data.timer - timer_dec), "âåú ðòïôïôéðá");
		}
	...
	send_to_char(buf, ch);
	sprintf(buf, "÷ÒÅÍÑ × ÒÅÎÔÅ: %ld ÔÉËÏ×.\r\n", timer_dec);
	send_to_char(buf, ch);
	sprintf(buf, "ðÒÅÄÍÅÔÏ×: %d. óÔÏÉÍÏÓÔØ: (%d × ÄÅÎØ) * (%1.2f ÄÎÅÊ) = %d. éîçòéäéåîôù îå ÷ù÷ïäñôóñ.\r\n",
...
void Crash_listrent(CHAR_DATA * ch, char *name)
{
	...
	if (index < 0)
	{
		send_to_char("îÅÔ ÔÁËÏÇÏ ÉÇÒÏËÁ.\r\n", ch);
		return;
	}
	...
	if (!SAVEINFO(index))
	{
		...
		{
			sprintf(buf, "%s ÎÅ ÉÍÅÅÔ ÆÁÊÌÁ ÒÅÎÔÙ.\r\n", CAP(name));
			send_to_char(buf, ch);
		}
		else
		{
			sprintf(buf, "%s ÎÁÈÏÄÉÔÓÑ × ÉÇÒÅ. óÏÄÅÒÖÉÍÏÅ ÆÁÊÌÁ ÒÅÎÔÙ:\r\n", CAP(name));
			send_to_char(buf, ch);
	...
	{
		sprintf(buf, "%s ÎÁÈÏÄÉÔÓÑ × ÒÅÎÔÅ. óÏÄÅÒÖÉÍÏÅ ÆÁÊÌÁ ÒÅÎÔÙ:\r\n", CAP(name));
		send_to_char(buf, ch);
		Crash_list_objects(ch, index);
	}
}
...
// ÒÁÚÏÂÒÁÔØÓÑ Ó ×ÏÚ×ÒÁÝÁÅÍÙÍ ÚÎÁÞÅÎÉÅÍ
// *******************  load_char_objects ********************
int Crash_load(CHAR_DATA * ch)
{
	...
	default:
		...
		send_to_char("\r\n** îÅÉÚ×ÅÓÔÎÙÊ ËÏÄ ÒÅÎÔÙ **\r\n"
					 "ðÒÏÂÌÅÍÙ Ó ×ÏÓÓÔÁÎÏ×ÌÅÎÉÅÍ ×ÁÛÉÈ ×ÅÝÅÊ ÉÚ ÆÁÊÌÁ.\r\n"
					 "ïÂÒÁÝÁÊÔÅÓØ ÚÁ ÐÏÍÏÝØÀ Ë âÏÇÁÍ.\r\n", ch);
		Crash_clear_objects(index);
		return (1);
		break;
	}
	...
	cost = MAX(0, cost);
	// added by WorM (÷ÉÄÏÌÀÂ) 2010.06.04 ÓÕÍÍÁ ÐÏÔÒÁÞÅÎÎÁÑ ÎÁ ÎÁÊÍ(×ÏÚ×ÒÁÝÁÅÔÓÑ ÐÒÉ ËÒÅÛÅ)
	...
	// end by WorM
  	// âÅÓÐÌÁÔÎÁÑ ÒÅÎÔÁ, ÅÓÌÉ ×ÙÊÔÉ × ÔÅÞÅÎÉÅ 2 ÞÁÓÏ× ÐÏÓÌÅ ÒÅÂÕÔÁ ÉÌÉ ËÒÅÛÁ
	...
	{
		sprintf(buf, "%s** îÁ ÓÅÊ ÒÁÚ ÐÏÓÔÏÊ ÂÙÌ ÂÅÓÐÌÁÔÎÙÍ **%s\r\n", CCWHT(ch, C_NRM), CCNRM(ch, C_NRM));
		send_to_char(buf, ch);
	...
	{
		sprintf(buf, "%s÷Ù ÎÁÈÏÄÉÌÉÓØ ÎÁ ÐÏÓÔÏÅ %1.2f ÄÎÅÊ.\n\r"
				"%s"
				"÷ÁÍ ÐÒÅÄßÑ×ÉÌÉ ÓÞÅÔ ÎÁ %d %s ÚÁ ÐÏÓÔÏÊ (%d %s × ÄÅÎØ).\r\n"
				"îÏ ×ÓÅ, ÞÔÏ Õ ×ÁÓ ÂÙÌÏ - %ld %s... õ×Ù. ÷ÓÅ ×ÁÛÉ ×ÅÝÉ ÐÅÒÅÄÁÎÙ ÍÏÂÁÍ.%s\n\r",
				CCWHT(ch, C_NRM),
				num_of_days,
				RENTCODE(index) ==
				RENT_TIMEDOUT ?
				"÷ÁÓ ÐÒÉÛÌÏÓØ ÔÁÝÉÔØ ÄÏ ËÒÏ×ÁÔÉ, ÚÁ ÜÔÏ ÐÏÓÔÏÊ ÂÙÌ ÄÏÒÏÖÅ.\r\n"
	...
	else
	{
		if (cost)
		{
			sprintf(buf, "%s÷Ù ÎÁÈÏÄÉÌÉÓØ ÎÁ ÐÏÓÔÏÅ %1.2f ÄÎÅÊ.\n\r"
					"%s"
					"ó ×ÁÓ ÓÏÄÒÁÌÉ %d %s ÚÁ ÐÏÓÔÏÊ (%d %s × ÄÅÎØ).%s\r\n",
					CCWHT(ch, C_NRM),
					num_of_days,
					RENTCODE(index) ==
					RENT_TIMEDOUT ?
	...
	if (!fread(readdata, fsize, 1, fl) || ferror(fl) || !readdata)
	{
		...
		send_to_char("\r\n** ïÛÉÂËÁ ÞÔÅÎÉÑ ÆÁÊÌÁ ÏÐÉÓÁÎÉÑ ×ÅÝÅÊ **\r\n"
					 "ðÒÏÂÌÅÍÙ Ó ×ÏÓÓÔÁÎÏ×ÌÅÎÉÅÍ ×ÁÛÉÈ ×ÅÝÅÊ ÉÚ ÆÁÊÌÁ.\r\n"
					 "ïÂÒÁÝÁÊÔÅÓØ ÚÁ ÐÏÍÏÝØÀ Ë âÏÇÁÍ.\r\n", ch);
	...
	*(data + fsize) = '\0';
	...
	// ðÒÏ×ÅÒËÁ × ËÁËÏÍ ÆÏÒÍÁÔÅ ÚÁÐÉÓÁÎÁ ÉÎÆÏÒÍÁÃÉÑ Ï ÐÅÒÓÏÎÁÖÅ.
			...
			reccount > 0 && *data && *data != END_CHAR; reccount--, fsize++)
	{
		...
		{
			// æÏÒÍÁÔ ÎÏ×ÙÊ => ÉÓÐÏÌØÚÕÅÍ ÎÏ×ÕÀ ÆÕÎËÃÉÀ
			obj = read_one_object_new(&data, &error);
			if (!obj)
			{
				//send_to_char ("ïÛÉÂËÁ ÐÒÉ ÞÔÅÎÉÉ - ÞÔÅÎÉÅ ÐÒÅÄÍÅÔÏ× ÐÒÅÒ×ÁÎÏ.\r\n", ch);
				send_to_char("ïÛÉÂËÁ ÐÒÉ ÞÔÅÎÉÉ ÆÁÊÌÁ ÏÂßÅËÔÏ×.\r\n", ch);
		...
		{
			// æÏÒÍÁÔ ÓÔÁÒÙÊ => ÉÓÐÏÌØÚÕÅÍ ÓÔÁÒÕÀ ÆÕÎËÃÉÀ
		...
		{
			send_to_char("îÅÔ ÓÏÏÔ×ÅÔÓÔ×ÉÑ ÚÁÇÏÌÏ×ËÏ× - ÞÔÅÎÉÅ ÐÒÅÄÍÅÔÏ× ÐÒÅÒ×ÁÎÏ.\r\n", ch);
		...
		// × Ä×Á ÄÅÊÓÔ×ÉÑ, ÞÔÏÂÙ ÚÁÏÄÎÏ ÓÎÑÔØ É ÔÁÊÍÅÒ ÏÂËÁÓÔÁ
		...
		// ðÒÅÄÍÅÔ ÒÁÚ×ÁÌÉ×ÁÅÔÓÑ ÏÔ ÓÔÁÒÏÓÔÉ
		if (obj->get_timer() <= 0)
		{
			sprintf(buf, "%s%s ÒÁÓÓÙÐÁÌ%s ÏÔ ÄÌÉÔÅÌØÎÏÇÏ ÉÓÐÏÌØÚÏ×ÁÎÉÑ.\r\n",
		...
		//ÏÞÉÝÁÅÍ ZoneDecay ÏÂßÅÄËÉ
		if (obj->get_extra_flag(EExtraFlag::ITEM_ZONEDECAY))
		{
			sprintf(buf, "%s ÒÁÓÓÙÐÁÌ%s × ÐÒÁÈ.\r\n", cap.c_str(), GET_OBJ_SUF_2(obj));
			send_to_char(buf, ch);
			extract_obj(obj.get());
			continue;
		}
		//ÏÞÉÝÁÅÍ RepopDecay ÏÂßÅÄËÉ
		...
		{
			sprintf(buf, "%s ÒÁÓÓÙÐÁÌ%s, ËÁË ÚÁÐÒÅÝÅÎÎ%s ÄÌÑ ×ÁÓ.\r\n",
...
int save_char_objects(CHAR_DATA * ch, int savetype, int rentcost)
{
	...
	//CRYO-rent ÎÁÄÏ ÄÏÒÁÂÁÔÙ×ÁÔØ ÌÉÂÏ ×ÙËÉÄÙ×ÁÔØ ÎÁÆÉÇ
	...
	// × ÐÒÉÎÃÉÐÅ ÜËÓÔÒÁËÔÉÔØ ÚÄÅÓØ ÞÁÒÍÉÓÏ×ÙÊ ÛÍÏÔ × ÓÌÕÞÁÅ ÒÅÂÕÔÁ - ÓÍÙÓÌÁ ÎÏÌØ
	if (savetype != RENT_CRASH)
	{
		for (j = 0; j < NUM_WEARS; j++)
		{
...
void Crash_rent_deadline(CHAR_DATA * ch, CHAR_DATA * recep, long cost)
{
	...
	if (!cost)
	{
		send_to_char("ôÙ ÓÍÏÖÅÛØ ÖÉÔØ Õ ÍÅÎÑ ÄÏ ×ÔÏÒÏÇÏ ÐÒÉÛÅÓÔ×ÉÑ.\r\n", ch);
		return;
	}
	...
	if (depot_cost)
	{
		send_to_char(ch, "\"úÁ ×ÅÝÉ × ÈÒÁÎÉÌÉÝÅ ÐÒÉÄÅÔÓÑ ÄÏÐÌÁÔÉÔØ %ld %s.\"\r\n", depot_cost, desc_count(depot_cost, WHAT_MONEYu));
		cost += depot_cost;
	}

	send_to_char(ch, "\"ðÏÓÔÏÊ ÏÂÏÊÄÅÔÓÑ ÔÅÂÅ × %ld %s.\"\r\n", cost, desc_count(cost, WHAT_MONEYu));
	rent_deadline = ((ch->get_gold() + ch->get_bank()) / cost);
	send_to_char(ch, "\"ô×ÏÉÈ ÄÅÎÅÇ È×ÁÔÉÔ ÎÁ %ld %s.\"\r\n", rent_deadline, desc_count(rent_deadline, WHAT_DAY));
}
		...
		if (Crash_is_unrentable(ch, obj))
		{
			...
			if (SetSystem::is_norent_set(ch, obj))
			{
				snprintf(buf, sizeof(buf),
						"$n ÓËÁÚÁÌ$g ×ÁÍ : \"ñ ÎÅ ÐÒÉÍÕ ÎÁ ÐÏÓÔÏÊ %s - ÔÒÅÂÕÅÔÓÑ Ä×Å É ÂÏÌÅÅ ×ÅÝÉ ÉÚ ÎÁÂÏÒÁ.\"",
						OBJN(obj, ch, 3));
			}
			else
			{
				snprintf(buf, sizeof(buf),
						"$n ÓËÁÚÁÌ$g ×ÁÍ : \"ñ ÎÅ ÐÒÉÍÕ ÎÁ ÐÏÓÔÏÊ %s.\"", OBJN(obj, ch, 3));
			}
...
// added by WorM (÷ÉÄÏÌÀÂ)
//ÐÒÏÃÅÄÕÒÁ ÄÌÑ ×Ù×ÏÄÁ ÓÔÏÉÍÏÓÔÉ ÒÅÎÔÙ ÏÄÎÏÊ É ÔÏÊ ÖÅ ÛÍÏÔËÉ(count ËÏÌ-×Á)
void Crash_report_rent_item(CHAR_DATA * ch, CHAR_DATA * recep, OBJ_DATA * obj, int count, int factor, int equip, int recursive)
{
	...
	if (obj)
	{
		if (CAN_WEAR_ANY(obj))
		{
			if (equip)
			{
				sprintf(bf, " (%d ÅÓÌÉ ÓÎÑÔØ)", GET_OBJ_RENT(obj) * factor * count);
			}
			else
			{
				sprintf(bf, " (%d ÅÓÌÉ ÎÁÄÅÔØ)", GET_OBJ_RENTEQ(obj) * factor * count);
			}
		}
		else
		{
		...
		if (!Crash_is_unrentable(ch, obj))
		{
			...
			*cost += MAX(0, ((equip ? GET_OBJ_RENTEQ(obj) : GET_OBJ_RENT(obj)) * factor));
			if (display)
			{
				...
				{
					if (!recursive)
						act("$n ÓËÁÚÁÌ$g ÷ÁÍ : \"ïÄÅÔ$W ÓÐÁÔØ ÂÕÄÅÛØ? èÍ.. îÕ ÓÍÏÔÒÉ, ÔÏÇÄÁ ÄÅÛÅ×ÌÅ ×ÏÚØÍÕ\"", FALSE, recep, 0, ch, TO_VICT);
					else
						act("$n ÓËÁÚÁÌ$g ÷ÁÍ : \"üÔÏ Ñ × ÞÕÌÁÎÅ ÚÁÐÒÕ:\"", FALSE,
							recep, 0, ch, TO_VICT);
				}
				if (CAN_WEAR_ANY(obj))
				{
					if (equip)
						sprintf(bf, " (%d ÅÓÌÉ ÓÎÑÔØ)", GET_OBJ_RENT(obj) * factor);
					else
						sprintf(bf, " (%d ÅÓÌÉ ÎÁÄÅÔØ)", GET_OBJ_RENTEQ(obj) * factor);
				}
				else
					*bf = '\0';
				sprintf(buf, "%s - %d %s%s ÚÁ %s %s",
						recursive ? "" : CCWHT(ch, C_SPR),
			...
			// added by WorM (÷ÉÄÏÌÀÂ)
			//ÇÒÕÐÐÉÒÕÅÔ ×Ù×ÏÄ ÐÒÉ ÒÅÎÔÅ ÎÁ ÐÏÄÏÂÉÉ:
			// - 2700 ËÕÎ (900 ÅÓÌÉ ÎÁÄÅÔØ) ÚÁ ËÒÁÓÎÙÊ ÐÕÚÙÒÅË [9]
				...
				*cost += MAX(0, ((equip ? GET_OBJ_RENTEQ(i) : GET_OBJ_RENT(i)) * factor));
				if (display)
				{
					if (*nitems == 1)
					{
						...
						{
							act("$n ÓËÁÚÁÌ$g ×ÁÍ : \"üÔÏ Ñ × ÞÕÌÁÎÅ ÚÁÐÒÕ:\"", FALSE, recep, 0, ch, TO_VICT);
						}
					}
...
// end by Dikiy
	...
	*totalcost = min_rent_cost(ch) * factor;
	...
	if (!numitems)
	{
		act("$n ÓËÁÚÁÌ$g ×ÁÍ : \"îÏ Õ ÔÅÂÑ ×ÅÄØ ÎÉÞÅÇÏ ÎÅÔ! ðÒÏÓÔÏ ÎÁÂÅÒÉ \"ËÏÎÅÃ\"!\"", FALSE, receptionist, 0, ch, TO_VICT);
		return (FALSE);
	}
	...
	{
		sprintf(buf,
			"$n ÓËÁÚÁÌ$g ×ÁÍ : \"éÚ×ÉÎÉÔÅ, ÎÏ Ñ ÎÅ ÍÏÇÕ ÈÒÁÎÉÔØ ÂÏÌØÛÅ %d ÐÒÅÄÍÅÔÏ×.\"\r\n"
			"$n ÓËÁÚÁÌ$g ×ÁÍ : \"÷ ÄÁÎÎÙÊ ÍÏÍÅÎÔ ÉÈ Õ ×ÁÓ %ld.\"", MAX_SAVED_ITEMS, numitems);
	...
	if (display)
	{
		...
		{
			sprintf(buf,
					"$n ÓËÁÚÁÌ$g ×ÁÍ : \"é ÅÝÅ %d %s ÍÎÅ ÎÁ ÓÂÉÔÅÎØ Ó ÍÅÄÏ×ÕÈÏÊ :)\"",
					min_rent_cost(ch) * factor, desc_count(min_rent_cost(ch) * factor, WHAT_MONEYu));
			act(buf, FALSE, receptionist, 0, ch, TO_VICT);
		}

		sprintf(buf, "$n ÓËÁÚÁÌ$g ×ÁÍ : \"÷ ÓÕÍÍÅ ÜÔÏ ÓÏÓÔÁ×ÉÔ %d %s %s.\"",
			*totalcost, desc_count(*totalcost, WHAT_MONEYu),
				(factor == RENT_FACTOR ? "× ÄÅÎØ " : ""));
		act(buf, FALSE, receptionist, 0, ch, TO_VICT);

		if (MAX(0, *totalcost / divide) > ch->get_gold() + ch->get_bank())
		{
			act("\"...ËÏÔÏÒÙÈ Õ ÔÅÂÑ ÏÔÒÏÄÑÓØ ÎÅ ÂÙÌÏ.\"", FALSE, receptionist, 0, ch, TO_VICT);
			return (FALSE);
		}
		...
		*totalcost = MAX(0, *totalcost / divide);
		if (divide == 2)
		{
			act("$n ÓËÁÚÁÌ$g ×ÁÍ : \"ôÁË ÕÖ É ÂÙÔØ, Ñ ÓËÏÝÕ ÔÅÂÅ ÐÏÌÏ×ÉÎÕ.\"",
...
int gen_receptionist(CHAR_DATA * ch, CHAR_DATA * recep, int cmd, char* /*arg*/, int mode)
{
	...
	if (!CMD_IS("offer") && !CMD_IS("ÐÒÅÄÌÏÖÅÎÉÅ")
			&& !CMD_IS("rent") && !CMD_IS("ÐÏÓÔÏÊ")
			&& !CMD_IS("quit") && !CMD_IS("ËÏÎÅÃ")
			&& !CMD_IS("settle") && !CMD_IS("ÐÏÓÅÌÉÔØÓÑ"))
		return (FALSE);

	save_room = ch->in_room;

	if (CMD_IS("ËÏÎÅÃ") || CMD_IS("quit"))
	{
	...
	if (!AWAKE(recep))
	{
		sprintf(buf, "%s ÎÅ × ÓÏÓÔÏÑÎÉÉ ÇÏ×ÏÒÉÔØ Ó ×ÁÍÉ...\r\n", HSSH(recep));
		send_to_char(buf, ch);
		return (TRUE);
	}
	if (!CAN_SEE(recep, ch))
	{
		act("$n ÓËÁÚÁÌ$g : \"îÅ ÌÀÂÌÀ ÇÏ×ÏÒÉÔØ Ó ÔÅÍÉ, ËÏÇÏ Ñ ÎÅ ×ÉÖÕ!\"", FALSE, recep, 0, 0, TO_ROOM);
		return (TRUE);
	}
	if (Clan::InEnemyZone(ch))
	{
		act("$n ÓËÁÚÁÌ$g : \"þÕÖÁËÁÍ ÚÄÅÓØ ÎÅ ÍÅÓÔÏ!\"", FALSE, recep, 0, 0, TO_ROOM);
		return (TRUE);
	}
	if (RENTABLE(ch))
	{
		send_to_char("÷ Ó×ÑÚÉ Ó ÂÏÅ×ÙÍÉ ÄÅÊÓÔ×ÉÑÍÉ Ü×ÁËÕÁÃÉÑ ×ÒÅÍÅÎÎÏ ÐÒÅËÒÁÝÅÎÁ.\r\n", ch);
		return (TRUE);
	}
	if (ch->get_fighting())
	{
	...
	if (free_rent)
	{
		act("$n ÓËÁÚÁÌ$g ×ÁÍ : \"óÅÇÏÄÎÑ ÓÐÉÍ ÎÁÈÁÌÑ×Õ.  îÁÂÅÒÉÔÅ ÐÒÏÓÔÏ \"ËÏÎÅÃ\".\"",
			FALSE, recep, 0, ch, TO_VICT);
		return (1);
	}
	if (CMD_IS("rent") || CMD_IS("ÐÏÓÔÏÊ"))
	{
		...
		if (!rentshow)
		{
			if (mode == RENT_FACTOR)
				sprintf(buf,
						"$n ÓËÁÚÁÌ$g ×ÁÍ : \"äÎÅ×ÎÏÊ ÐÏÓÔÏÊ ÏÂÏÊÄÅÔÓÑ ÔÅÂÅ × %d %s.\"",
						cost, desc_count(cost, WHAT_MONEYu));
			else if (mode == CRYO_FACTOR)
				sprintf(buf,
						"$n ÓËÁÚÁÌ$g ×ÁÍ : \"äÎÅ×ÎÏÊ ÐÏÓÔÏÊ ÏÂÏÊÄÅÔÓÑ ÔÅÂÅ × %d %s (ÚÁ ÐÏÌØÚÏ×ÁÎÉÅ ÈÏÌÏÄÉÌØÎÉËÏÍ :)\"",
						cost, desc_count(cost, WHAT_MONEYu));
			act(buf, FALSE, recep, 0, ch, TO_VICT);

			if (cost > ch->get_gold() + ch->get_bank())
			{
				act("$n ÓËÁÚÁÌ$g ×ÁÍ : '..ÎÏ ÔÁËÏÊ ÇÏÌÙÔØÂÅ, ËÁË ÔÙ, ÜÔÏ ÎÅ ÐÏ ËÁÒÍÁÎÕ.'",
					FALSE, recep, 0, ch, TO_VICT);
				return (TRUE);
			}
		...
		if (mode == RENT_FACTOR)
		{
			act("$n ÚÁÐÅÒ$q ×ÁÛÉ ×ÅÝÉ × ÓÕÎÄÕË É ÐÏ×ÅÌ$g × ÔÅÓÎÕÀ ËÁÍÏÒËÕ.", FALSE, recep, 0, ch, TO_VICT);
		...
		{
			act("$n ÚÁÐÅÒ$q ×ÁÛÉ ×ÅÝÉ × ÓÕÎÄÕË É ÐÏ×ÅÌ$g × ÔÅÓÎÕÀ ËÁÍÏÒËÕ.\r\n"
				"âÅÌÙÊ ÐÒÉÚÒÁË ÐÏÑ×ÉÌÓÑ × ËÏÍÎÁÔÅ, ÏÂÄÁ× ×ÁÓ ÈÏÌÏÄÏÍ...\r\n"
				"÷Ù ÐÏÔÅÒÑÌÉ Ó×ÑÚØ Ó ÏËÒÕÖÁÀÝÉÍÉ ×ÁÓ...", FALSE, recep, 0, ch, TO_VICT);
		...
		if ((save_room == r_helled_start_room)
			...
			act("$n ÐÒÏ×ÏÄÉÌ$g $N3 ÍÏÝÎÙÍ ÐÉÎËÏÍ ÎÁ Ó×ÏÂÏÄÎÕÀ ÌÁ×ËÕ.", FALSE, recep, 0, ch, TO_ROOM);
		else
		{
			act("$n ÐÏÍÏÇ$q $N2 ÏÔÏÊÔÉ ËÏ ÓÎÕ.", FALSE, recep, 0, ch, TO_NOTVICT);
			GET_LOADROOM(ch) = GET_ROOM_VNUM(save_room);
		}
		Clan::clan_invoice(ch, false);
		extract_char(ch, FALSE);
	}
	else if (CMD_IS("offer") || CMD_IS("ÐÒÅÄÌÏÖÅÎÉÅ"))
	{
		Crash_offer_rent(ch, recep, TRUE, mode, &cost);
		act("$N ÐÒÅÄÌÏÖÉÌ$G $n2 ÏÓÔÁÎÏ×ÉÔØÓÑ Õ Î$S.", FALSE, ch, 0, recep, TO_ROOM);
	}
	else
	{
		if ((save_room == r_helled_start_room)
			|| (save_room == r_named_start_room)
			|| (save_room == r_unreg_start_room))
			act("$N ÓËÁÚÁÌ$G : \"ëÕÄÁ ÖÅ ÔÙ ÄÅÎÅÛØÓÑ ÏÔ ÍÅÎÑ?\"", FALSE, ch, 0, recep, TO_CHAR);
		else
		{
			act("$n ÐÒÅÄÌÏÖÉÌ$g $N2 ÐÏÓÅÌÉÔØÓÑ Õ Î$s.", FALSE, recep, 0, ch, TO_NOTVICT);
			act("$N ÓËÁÚÁÌ$G : \"ëÏÎÅÞÎÏ, ÐÒÉÍÅÍ × ÌÀÂÏÅ ×ÒÅÍÑ É ÐÏÞÔÉ × ÌÀÂÏÍ ÓÏÓÔÏÑÎÉÉ!\"", FALSE, ch, 0, recep, TO_CHAR);
...
// * óÅÊ× ÐÒÉ ÐÌÁÎÏ×ÏÍ ÒÅÂÕÔÅ/ÏÓÔÁÎÏ×ËÅ Ó ÔÁÊÍÅÒÏÍ != 0.
void Crash_save_all_rent(void)
{
	// shapirus: ÐÒÏÈÏÄÉÍ ÎÅ ÐÏ ÓÐÉÓËÕ ÄÅÓËÒÉÐÔÏÒÏ×,
	// Á ÐÏ ÓÐÉÓËÕ ÞÁÒÏ×, ÞÔÏÂÙ ÓÏÈÒÁÎÉÔØ ÚÁÏÄÎÏ É ÔÅÈ,
	// ËÔÏ ÐÅÒÅÄ ÒÅÂÕÔÏÍ ÕÛÅÌ × ìä Ó ÃÅÌØÀ ÓÏÈÒÁÎÉÔØ
	// Ó×ÏÉ ÇÒÑÚÎÙÅ ÄÅÎÅÖËÉ.
