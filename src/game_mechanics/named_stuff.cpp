...
bool check_named(CharData *ch, const ObjData *obj, const bool simple) {
	...
	if (it != stuff_list.end()) {
		...
		if (IS_CHARMICE(ch)) // Чармисы тоже могут работать с именными вещами
		{
			...
			if (it->second->uid == GET_ID(master)) // Чармис владельца предмета
			{
				return false;
		...
		if (IS_IMMORTAL(ch)) // Имм
			return false;
		if (it->second->uid == GET_ID(ch))//Это владелец предмета
			return false;
...
void receive_items(CharData *ch, CharData *mailman) {
	...
	for (StuffListType::const_iterator it = stuff_list.begin(), iend = stuff_list.end(); it != iend; ++it) {
		if ((it->second->uid == GET_ID(ch)) || (!strcmp(GET_EMAIL(ch), it->second->mail.c_str()))) {
			if ((r_num = GetObjRnum(it->first)) < 0) {
