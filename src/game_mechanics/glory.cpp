		...
		switch (stat) {
			...
			case G_CHA:
				if (ch->GetInbornCha() == ch->desc->glory->olc_cha)
					stop = true;
				break;
			default: log("Glory: невалидный номер стат: %d (uid: %ld, name: %s)", stat, GET_ID(ch), GET_NAME(ch));
				stop = true;
		}
		if (stop) {
...
bool parse_remove_stat(CharData *ch, int stat) {
	...
	log("Glory: не найден стат при вычитании в олц (stat: %d, uid: %ld, name: %s)", stat, GET_ID(ch), GET_NAME(ch));
	return true;
}
...
void transfer_stats(CharData *ch, CharData *god, const std::string& name, char *reason) {
	...
	DescriptorData *k = DescriptorByUid(GET_ID(ch));
	if (k) {
...
void hide_char(CharData *vict, CharData *god, char const *const mode) {
	...
	bool ok = true;
	auto it = glory_list.find(GET_ID(vict));
	if (it != glory_list.end()) {
...
int get_spend_glory(CharData *ch) {
	auto i = glory_list.find(GET_ID(ch));
	if (i != glory_list.end()) {
