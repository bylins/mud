...
ACMD(do_attach)
{
	...
	{
		if (iswdigit(*targ_name) && !strchr(targ_name, '.'))
		{
...
int remove_trigger(SCRIPT_DATA * sc, char *name, TRIG_DATA ** trig_addr)
{
	...
	if ((cname = strstr(name, ".")) || (!iswdigit(*name)))
	{
		string = TRUE;
		if (cname)
		{
...
int is_num(char *num)
{
	while (*num && (iswdigit(static_cast<unsigned char>(*num)) || *num == '-'))
		num++;
