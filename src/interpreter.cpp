...
void check_hiding_cmd(CHAR_DATA *ch, int percent) {
	...
	if (affected_by_spell(ch, SPELL_HIDE)) {
		if (percent == -2) {
			if (AFF_FLAGGED(ch, EAffectFlag::AFF_SNEAK)) {
				remove_hide = number(1, skill_info[SKILL_SNEAK].difficulty) >
					CalculateCurrentSkill(ch, SKILL_SNEAK, 0);
			} else {
				percent = 500;
			}
		}
		...
		if (percent == -1) {
			...
			remove_hide = number(1, percent) > CalculateCurrentSkill(ch, SKILL_HIDE, 0);
		}
...
bool who_spamcontrol(CHAR_DATA *ch, unsigned short int mode = WHO_LISTALL) {
	...
	mana = MIN(WHO_MANA_MAX,
			   mana + (ctime - last) * WHO_MANA_REST_PER_SECOND
				   + (ctime - last) * WHO_MANA_REST_PER_SECOND * (RENTABLE(ch) ? 1 : 0));
