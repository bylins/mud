...
olc_data::olc_data()
	...
	storage(0),
	extra_desc_num(-1),
	next_extra_desc(false),
	temp_extra_desc_text(0)
{
...
olc_data::~olc_data()
{
	if (temp_extra_desc_text)
	{
		free(temp_extra_desc_text);
	}
}

/**
* Выход из редактирования текста экстра-описания.
*/
void olc_data::exit_exdesc_edit()
{
	if (temp_extra_desc_text && *temp_extra_desc_text)
	{
		description.text = temp_extra_desc_text;
		free(temp_extra_desc_text);
		temp_extra_desc_text = 0;
	}
}
...
/**
* Очистка полей экстра-описания.
*/
void olc_data::clear_exdesc()
{
	description.key.clear();
	description.text.clear();
	extra_desc_num = -1;
	next_extra_desc = false;
	// temp_extra_desc_text чистится своим ходом
}

/**
* Вывод основного меню экстра-описаний (объект/комната).
*/
void olc_data::main_exdesc_menu(CHAR_DATA *ch)
{
	this->exit_exdesc_edit();
	...
	snprintf(buf, MAX_STRING_LENGTH,
			...
			grn, nrm, yel,
			description.key.empty() ? "<NONE>" : description.key.c_str(), grn, nrm,
			yel, description.text.empty() ? "<NONE>" : description.text.c_str(), grn, nrm);

	strcat(buf, !next_extra_desc ? "<NOT SET>\r\n" : "Set.\r\n");
	strcat(buf, "Enter choice (0 to quit) : ");
	send_to_char(buf, ch);
}

void olc_data::setup_exdesc(ExtraDescSystem::PtrType &extra_desc)
{
	if (extra_desc)
	{
		extra_desc_num = 0;
		description = extra_desc->olc_get_node(0);
		next_extra_desc = extra_desc->olc_get_next(0);
	}
}

void olc_data::set_exdesc_key(const char *arg)
{
	if (arg && *arg)
	{
		description.key = arg;
...
void olc_data::exit_exdesc_menu(DESCRIPTOR_DATA *d, int mode)
{
	if (!description.key.empty() && !description.text.empty())
	{
		if (extra_desc_num >= 0)
		{
			// редактировали существующее описание
			if (mode == REDIT_MODE)
			{
				OLC_ROOM(d)->extra_desc->olc_set_node(extra_desc_num, description);
			}
			else
			{
				OLC_OBJ(d)->extra_desc->olc_set_node(extra_desc_num, description);
			}
		}
		else
		{
			...
			if (mode == REDIT_MODE)
			{
				ExtraDescSystem::add(OLC_ROOM(d), description.key.c_str(),
						description.text.c_str());
			}
			else
			{
				ExtraDescSystem::add(OLC_OBJ(d), description.key.c_str(),
						description.text.c_str());
			}
		}
	}
...
void olc_data::setup_exdesc_text_edit(DESCRIPTOR_DATA *d)
{
	SEND_TO_Q("Введите экстраописание: (/s сохранить /h помощь)\r\n\r\n", d);
	d->backstr = NULL;
	if (temp_extra_desc_text)
	{
		SEND_TO_Q(temp_extra_desc_text, d);
		d->backstr = str_dup(temp_extra_desc_text);
	}
	d->str = &temp_extra_desc_text;
	d->max_str = 4096;
	d->mail_to = 0;
}

void olc_data::next_exdesc_menu(DESCRIPTOR_DATA *d, int mode)
{
	if (description.key.empty() || description.text.empty())
	{
		send_to_char
				("Вы не можете редактировать следующее экстраописание,"
				" не завершив текущее.\r\n", d->character);
	}
	else
	{
		...
		{
			// редактировали существующее описание
			ExtraDescSystem::PtrType extra_desc =
					mode == REDIT_MODE ? OLC_ROOM(d)->extra_desc : OLC_OBJ(d)->extra_desc;

			extra_desc->olc_set_node(extra_desc_num, description);
			if (next_extra_desc)
			{
				// сетим следующее, если оно есть
				++extra_desc_num;
				description = extra_desc->olc_get_node(extra_desc_num);
				next_extra_desc = extra_desc->olc_get_next(extra_desc_num);
			}
			else
			{
				// или чистим все для последующего добавления нового
				clear_exdesc();
			}
		}
		else
		{
			...
			if (mode == REDIT_MODE)
			{
				ExtraDescSystem::add(OLC_ROOM(d), description.key.c_str(), description.text.c_str());
			}
			else
			{
				ExtraDescSystem::add(OLC_OBJ(d), description.key.c_str(), description.text.c_str());
			}
			clear_exdesc();
		}
	}
