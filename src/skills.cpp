...
int calculate_skill(CHAR_DATA * ch, const ESkill skill_no, CHAR_DATA * vict)
{
	...
	case SKILL_SENSE:      // найти
		...
		percent = 0;
            ...
            victim_sav = -GET_REAL_HEIGHT(ch);
            //срезаем скил из-за большой разницы в мортах
            if (GET_REMORT(victim) > GET_REMORT(ch) + 5 + MAX(0,(GET_REMORT(victim) - GET_REMORT(ch))/3))
            {
                victim_sav -= (80 +GET_REMORT(victim)*5) / 2;
            }
            // если разница по левелам больше чем разница для группы у того кто ищет - срезаем умение
            if ((GET_LEVEL(victim) - (GET_LEVEL(ch)+grouping[static_cast<int>(GET_CLASS(ch))][static_cast<int>(GET_REMORT(ch))])) >= 0)
            {
                victim_sav -= (GET_LEVEL(victim) - GET_LEVEL(ch))*5 / 2;
            }
            
            if (vict)
		{
                    victim_modi += GET_REAL_SIZE(vict) + GET_REAL_WEIGHT(vict);
                    if (AFF_FLAGGED(vict, EAffectFlag::AFF_NOTRACK)
				|| ROOM_FLAGGED(ch->in_room, ROOM_NOTRACK))
			{
				victim_modi = victim_modi / 2;
			}
		}
		break;
        }
...
//        victim_sav = GET_SAVE(vict, SAVING_REFLEX);
	...
	case SKILL_HEAL:   // !heal! нигде не используеться
		break;
	...
	case SKILL_NOPARRYHIT: //скрытый удар
        {
		bonus = dex_bonus(GET_REAL_DEX(ch));
		break;
        }
	...
	case SKILL_WARCRY:   //боевой клич
		break;
	case SKILL_COURAGE:  //ярость
		break;
	...
	case SKILL_CREATE_WAND:
		break;
	case SKILL_LOOK_HIDE:    //подсмотреть
        {
	...
	case SKILL_ARMORED:
		break;
	case SKILL_DRUNKOFF:     //опохмелиться
        {
	...
	case SKILL_FIRE:         //разжечь костер
        {
