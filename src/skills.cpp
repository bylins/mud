...
int calculate_skill(CHAR_DATA * ch, int skill_no, int max_value, CHAR_DATA * vict)
{
	...
	case SKILL_BACKSTAB:	//заколоть
		...
		if (vict)
		{
			...
			else if (AFF_FLAGGED(vict, EAffectFlags::AFF_AWARNESS))
				victim_modi -= 30;
...
//		victim_modi = 100;
	...
	case SKILL_STEAL:	// Украсть
		...
		if (vict)
		{
			...
			{
				victim_modi -= int_app[GET_REAL_INT(vict)].observation;
				if (AFF_FLAGGED(vict, EAffectFlags::AFF_AWARNESS))
					bonus -= 30;
			}
		}
		break;
	...
	case SKILL_CHOPOFF:  // подножка
		...
		if (vict)
		{
			...
			if (GET_POS(vict) < POS_SITTING)
				bonus -= 50;
			if (AWAKE(vict) || AFF_FLAGGED(vict, EAffectFlags::AFF_AWARNESS) || MOB_FLAGGED(vict, MOB_AWAKE))
				victim_modi -= 20;
			if (GET_AF_BATTLE(vict, EAF_AWAKE))
				victim_modi -= calculate_awake_mod(ch, vict);
//			victim_modi -= int_app[GET_REAL_INT(vict)].observation;
	...
	case SKILL_STRANGLE: // удавить
			...
			(MOB_FLAGGED(vict, MOB_AWARE) || AFF_FLAGGED(vict, EAffectFlags::AFF_AWARNESS) || AWAKE(vict) ))
...
//			victim_modi += percent - skill_info[skill_no].max_percent;
		...
		if (AFF_FLAGGED(ch, EAffectFlags::AFF_DEAFNESS))
