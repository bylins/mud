...
public:
	...
	auto Enqueue(Func&& task) -> std::future<typename std::invoke_result<Func()>::type>;
...
template<typename Func>
auto ThreadPool::Enqueue(Func&& task) -> std::future<typename std::invoke_result<Func()>::type> {
