	...
	while (get_line(zt_file, tmp))
	{
		if (!strn_cmp(tmp, "ИМЯ", 3))
		{
			...
			if (!strn_cmp(tmp, "ТИПЫ", 4))
			{
				...
				for (i = 4; tmp[i] != '\0'; i++)
				{
					if (!isdigit_wrapped(tmp[i]) && !a_isspace(tmp[i]))
					{
						log("Corrupted file : ztypes.lst");
						return;
					}
...
//		if (get_line(zt_file, tmp)); не уверен что тут хотел автор -- Krodo
		...
		for (j = 4; tmp[j] != '\0'; j++)
		{
			...
			for (; tmp[j] != '\0' && isdigit_wrapped(tmp[j]); j++);
			j--;
		}
		i++;
	}
...
//		if (get_line(zt_file, tmp)); аналогично тому, что выше
		...
		for (j = 4, n = 0; tmp[j] != '\0'; j++)
		{
			if (a_isspace(tmp[j]))
				continue;
			for (k = 0; tmp[j] != '\0' && isdigit_wrapped(tmp[j]); j++)
				itype_num[k++] = tmp[j];
			itype_num[k] = '\0';
...
void OBJ_DATA::init_set_table()
{
	...
	while (!fp.eof())
	{
		...
		if (cppstr[0] == '#')
		{
			...
			if (cppstr.empty() || !isdigit_wrapped(cppstr[0]))
			{
