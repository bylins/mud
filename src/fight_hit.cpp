...
bool Damage::magic_shields_dam(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if (AFF_FLAGGED(victim, AFF_MAGICGLASS) && dmg_type == MAGE_DMG)
	{
		int pct = 6;
...
void Damage::process_death(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if (killer)
	{
		...
		else if ((AFF_FLAGGED(killer, AFF_HORSE) || AFF_FLAGGED(killer, AFF_CHARM) || MOB_FLAGGED(killer, MOB_ANGEL)) && killer->master)
