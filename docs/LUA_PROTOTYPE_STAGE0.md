...
- Binding: sol2.
- VM: LuaJIT.
- Build integration: optional Meson `luajit_prototype` mode.
...
```sh
meson setup build -Dluajit_prototype=true
ninja -C build
```

Meson downloads `sol2` through `subprojects/sol2.wrap` when no system package is available. Use
`-Dsol2_root=/path/to/sol2` only to force a local copy.

The default build keeps the command registered, but it replies that the Lua prototype is not compiled in.
