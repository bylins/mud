...
В OLC-меню Lua-триггера доступен пункт `Автоформат Lua`. Он форматирует текущий текст встроенной библиотекой EmmyLuaCodeStyle с отступом в 2 пробела. Форматирование запускается только явно: просмотр и сохранение сами текст не меняют. Форматтер работает в отдельном рабочем потоке, поэтому игровой цикл не останавливается; запросы от разных редакторов выполняются последовательно в порядке очереди. До получения результата текущее меню редактора ожидает завершения операции, но из него можно выйти командой `Q`. Если исходник содержит синтаксическую ошибку или результат нельзя сохранить в KOI8-R, исходный OLC-буфер остается без изменений. В сборке без LuaJIT пункт не показывается; встроенный форматтер можно отключить опцией Meson `-Dlua_formatter=false`.
