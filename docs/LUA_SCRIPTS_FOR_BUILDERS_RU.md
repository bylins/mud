...
Lua-поддержка сейчас является прототипом и работает только в сборке с включенной опцией `luajit`, например `-Dluajit=builtin` или `-Dluajit=system`. Обычная сборка без LuaJIT должна продолжать читать DG-триггеры как раньше.
...
В OLC-меню Lua-триггера доступен пункт `Проверить синтаксис и отформатировать Lua`. Он форматирует текущий текст встроенной библиотекой EmmyLuaCodeStyle с отступом в 2 пробела. При выходе из OLC с сохранением Lua-код проверяется и форматируется автоматически. Если проверка или форматирование завершается ошибкой, OLC показывает сообщение об ошибке, сохраняет исходную версию кода без форматирования и закрывает редактор. Форматтер работает в отдельном рабочем потоке, поэтому игровой цикл не останавливается; запросы от разных редакторов выполняются последовательно в порядке очереди. Во время автоматического форматирования команда `Q` отменяет форматирование, сохраняет исходный код и закрывает редактор. При разрыве соединения после подтверждения сохранения исходный код также сохраняется. При ручном запуске форматирования из меню ожидание можно отменить командой `Q` без сохранения. В сборке без LuaJIT пункт не показывается; встроенный форматтер можно отключить опцией Meson `-Dlua_formatter=false`.
